...
| motion_delta | `[B,T,12]` | `motion_tensor.v2`; 12 confirmed end-to-end by real bounded export (NPZ `[1,6,12]`) |
...
Spatial predictive per-frame width **= 46** (`4+2+12+18+10`; corrected from 44/legacy-10-D motion, verified by real bounded export — see `motion_dimension_contract.json`). Loader shapes confirmed: image `[4,6,3,64,64]`, logits `[4,10]`.
