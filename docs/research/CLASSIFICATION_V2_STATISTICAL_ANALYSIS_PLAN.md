...
- **Generalization across conditions → recording date.** The outer protocol is **`FOUR_FOLD_DATE_GROUPED_OUTER_CV`** (whole calendar dates via `OUTER_DATE_GROUP_ID`; **NOT** full leave-one-date-out): F1 holds out `291119`, F2 `301119`, F3 `281119`, F4 pools the 11 small legacy tokens. Reported uncertainty is over dates; **descriptive per-date metrics are still reported for all 13 groups**.
...
- **Cross-length comparisons** (target-view length `T6–T16` or causal-history length `H6–H24`, treated as **two separate families**) report **both** `ALL_ELIGIBLE` and `COMMON_MATCHED_COHORT`; a length gain counts only if it **survives the matched cohort**, evaluated **after** the overlapping-window **training-mass correction** (fixed windows per native-unit, or `sample_weight = native_unit_class_weight / windows_from_same_native_unit`; native-unit mass audited before/after). An `ALL_ELIGIBLE`-only difference is treated as a population/support artifact, not a gain.
...
- The ablation families (loss L0–L7; **target-view length T6–T16 and causal-history length H6–H24 as two separate families**; social variants) are declared families in advance; no uncorrected fishing across them.
...
| eval population | for length comparisons: ALL_ELIGIBLE **and** COMMON_MATCHED_COHORT (gain must survive the matched cohort) |
| per-date | Δmacro-F1 per outer date + worst-date (`FOUR_FOLD_DATE_GROUPED_OUTER_CV`) |
