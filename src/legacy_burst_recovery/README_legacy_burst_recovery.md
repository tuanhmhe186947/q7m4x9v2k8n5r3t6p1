...
```text
data\annotations\scene\mask.png
```
...
python -m legacy_burst_recovery.main ^
  --input-csv "old_burst_center_keyframes_nodup_videos.csv" ^
  --legacy-burst-bbox-csv "old_burst_all_keyframe_bboxes_nodup_videos.csv" ^
  --drive-root "G:\My Drive" ^
  --output-root "outputs\legacy_full_multigt_masked_nodup" ^
  ...
  --track-end-mode sample_0_6_12 ^
  --extract-crops ^
  --manual-review-csv "manual_review_stable.csv" ^
  --sequence-views sparse_3_0_6_12 dense_6_same_span dense_12_same_span full_dense_0_to_12 ^
  --resume ^
  --progress ^
  --log-file "outputs\legacy_full_multigt_masked_nodup.log" ^
  --flush-every 500
```
...
```cmd
python -c "import pandas as pd; p=r'outputs\legacy_full_multigt_masked_nodup\manual_review_apply_audit.csv'; a=pd.read_csv(p,low_memory=False); print(a.to_string(index=False))"
```
...
python -m legacy_burst_recovery.main ^
  ...
  --legacy-burst-bbox-csv "old_burst_all_keyframe_bboxes_nodup_videos.csv" ^
  --drive-root "G:\My Drive" ^
  --output-root "outputs\debug_one_tracklet" ^
  --detector-weights "models\detector\pig_detector_yolov8.pt" ^
  --scene-mask "data\annotations\scene\mask.png" ^
  --mask-filter-detections ^
  --mask-min-bbox-coverage 0.50 ^
...
python -m legacy_burst_recovery.main ^
  ...
  --legacy-burst-bbox-csv "old_burst_all_keyframe_bboxes_nodup_videos.csv" ^
  --drive-root "G:\My Drive" ^
  --output-root "outputs\legacy_test_20_full_legacy_burst" ^
  --detector-weights "models\detector\pig_detector_yolov8.pt" ^
  --scene-mask "data\annotations\scene\mask.png" ^
  --mask-filter-detections ^
  --mask-min-bbox-coverage 0.50 ^
  ...
  --max-rows 20 ^
  --progress ^
  --log-file "outputs\legacy_test_20_full_legacy_burst.log" ^
  --flush-every 10
```
...
```text
outputs\legacy_full_multigt_masked_nodup\legacy_dense_tracklet_map.csv
```
...
```text
outputs\legacy_full_multigt_masked_nodup\exports
```
...
```cmd
type "outputs\legacy_full_multigt_masked_nodup\progress_state.json"
```

Hoặc xem log:

```cmd
type "outputs\legacy_full_multigt_masked_nodup.log"
```
