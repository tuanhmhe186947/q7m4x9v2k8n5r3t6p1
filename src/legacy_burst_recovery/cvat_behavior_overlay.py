"""Apply CVAT k0 behavior authority to recovered legacy 16-frame bursts."""
...
from pig_behavior.data.cvat_native import load_all_cvat_tasks

PROPAGATION_POLICY = "cvat_k0_per_burst_pig_to_all_legacy_dense_frames"
LABEL_SOURCE = "cvat_native_k0_burst_authority"
MISSING_LABEL_SOURCE = "missing_cvat_k0_no_training_label"
...
def apply_cvat_k0_behavior_authority(
    ...
    *,
    ...
    """Overlay one k0 behavior label onto every dense row of its legacy burst.
    ...
    """
    if authority_slot < 0:
        raise ValueError("authority_slot must be non-negative")
    ...
    authority = prepared.loc[prepared["selected_slot"].eq(authority_slot)].copy()
    errors = _authority_errors(authority)
    if errors:
        raise ValueError("Invalid CVAT k0 behavior authority: " + "; ".join(errors))
...
def _prepare_cvat_rows(
    ...
    for task_dir in sorted(export_root.glob("task_*")):
        for relative in [Path("annotations.json"), Path("data/manifest.jsonl")]:
...
def _overlay_dense(
    ...
    out["legacy_behavior_before_cvat_k0"] = out["behavior"]
    ...
    out["legacy_behavior_changed_by_cvat_k0"] = (
        has_authority
    ...
    out["legacy_behavior_authority_status"] = "authoritative_k0"
    ...
    discrepancies["join_status"] = discrepancies["join_status"].map(
        {"left_only": "dense_missing_k0", "right_only": "k0_missing_dense"}
    )
    ...
    join_audit = {
        ...
        "dense_keys_missing_k0": int(key_join["_merge"].eq("left_only").sum()),
        "k0_keys_missing_dense": int(key_join["_merge"].eq("right_only").sum()),
        "k0_keys_extra_within_dense_groups": int(
            key_join.loc[
        ...
        "rows_excluded_missing_k0": int((~has_authority).sum()),
        "rows_changed_by_k0": int(out["legacy_behavior_changed_by_cvat_k0"].sum()),
        "behavior_keys_changed_by_k0": int(
            out.loc[
                out["legacy_behavior_changed_by_cvat_k0"],
                ["_authority_group_id", "_authority_pig_id"],
...
def _build_audit(
    *,
    ...
    warnings: list[str] = []
    if join_audit["dense_keys_missing_k0"]:
        warnings.append("dense keys without k0 were retained as rows and excluded")
    if join_audit["k0_keys_missing_dense"]:
        warnings.append("k0 objects without recovered dense tracklets were not fabricated")
    if disagreements:
        warnings.append("sampled k1..k5 labels disagree with k0 and remain audit-only")
    if sampled_duplicate_rows:
        warnings.append("duplicate non-authority sampled object keys were detected")
    if incomplete_anchor_keys:
        warnings.append("incomplete sampled six-anchor actor keys were detected")
    if groups_without_authority:
        warnings.append("CVAT groups without the selected authority slot were ignored")
    ...
    return {
        ...
        "counts": {
            ...
            "cvat_object_rows": int(len(prepared)),
            "cvat_groups": int(prepared["group_id"].nunique()),
            "cvat_groups_without_authority_slot": int(
        ...
        "cvat_groups_without_authority_slot": groups_without_authority,
        "source_files": source_files,
        "errors": [],
        "warnings": warnings,
    }
