"""Bounded union-context classifier controls for legacy L6 development."""
...
SHORT_CONFIG_SCHEMA = (
    ...
    "union_context_short_config.v1"
)
NORMALIZATION_SCHEMA = (
    "classification_v2.legacy_development_l6."
    "union_context_normalization.v1"
)
PREFLIGHT_SCHEMA = (
    "classification_v2.legacy_development_l6."
    "union_context_preflight.v1"
)
WHITELIST_SCHEMA = (
    "classification_v2.legacy_development_l6."
    "union_context_whitelist.v1"
)


@dataclass(frozen=True, slots=True)
class LegacyL6UnionContextConfig:
    """Hash-bound short actor-partner union-context experiment."""
...
@dataclass(frozen=True, slots=True)
class LegacyL6UnionContextOutcome:
    """Selected-checkpoint outputs for one union-context mode."""
...
def load_union_context_training_config(
    path: Path,
) -> LegacyL6UnionContextConfig:
    """Load and verify the immutable union-context experiment contract."""
    ...
    for section in ("parents", "inputs", "implementation"):
        for name, value in _object(payload[section], section).items():
            ...
            _validate_bound_file(
                ...
                str(spec["sha256"]),
                f"union context {section}.{name}",
            )
    return config


def fit_union_context_normalization(
    cache: Any,
    selection: TemporalLadderSelection,
) -> CachedModalityNormalizationState:
    """Fit feature statistics from unique available training slots only."""
    ...
    return fit_cached_modality_normalization(
        ...
        identity_field="union_context_window_slot_uid",
        schema_version=NORMALIZATION_SCHEMA,
    )


def build_union_context_view(
    ...
    *,
    ...
    """Align union features and availability to the frozen T6 view."""
...
def union_context_feature_whitelist(mode: str) -> dict[str, Any]:
    """Return the explicit 1,025-wide union-context model-X contract."""
...
def build_union_context_model(
    ...
    if observed != EXPECTED_PARAMETER_COUNT:
        raise ValueError(
            f"L6 union context model parameters={observed}!="
            f"{EXPECTED_PARAMETER_COUNT}"
        )
    return classifier


def preflight_union_context_mode(
    config: LegacyL6UnionContextConfig,
    mode: str,
) -> dict[str, Any]:
    ...
    if mode not in MODES:
        raise ValueError(f"unknown L6 union context mode={mode}")
    ...
    try:
        _, base, cache, selection = load_union_context_training_inputs(config)
        normalization = fit_union_context_normalization(cache, selection)
        view = build_union_context_view(
            base,
            cache,
            mode=mode,
            normalization=normalization,
        ...
        model = build_union_context_model(config)
        parameter_count = sum(
        ...
        with torch.inference_mode():
            ...
            missing_logits = model(
                ...
                time_delta=torch.from_numpy(missing_batch["time_delta"])
                .float(),
            )
        ...
        if output_shape != expected_shape:
            errors.append("L6 union context CPU forward shape drift")
        if missing_output_shape != expected_shape:
            errors.append("L6 union context missing shape drift")
        ...
        if cache_content.get("availability_patterns") != expected_patterns:
            errors.append("L6 union context availability pattern drift")
        expected_features = {
        ...
        if observed_features != expected_features:
            errors.append(
                f"L6 union context feature contract={observed_features}"
            )
    ...
    git_guard = union_context_training_git_guard(config)
    errors.extend(str(value) for value in git_guard["errors"])
    cuda_after = torch.cuda.is_initialized()
    if cuda_before or cuda_after:
        errors.append("L6 union context preflight initialized CUDA")
    valid = not errors
    return {
        "schema_version": PREFLIGHT_SCHEMA,
        "status": (
            "PASS_LEGACY_DEVELOPMENT_L6_UNION_CONTEXT_PREFLIGHT"
            if valid
            else "FAIL_LEGACY_DEVELOPMENT_L6_UNION_CONTEXT_PREFLIGHT"
        ),
        ...
        "feature_whitelist": union_context_feature_whitelist(mode),
...
def train_union_context_core(
    ...
    *,
...
) -> LegacyL6UnionContextOutcome:
    """Train one union-context control through the shared L6 loop."""

    if mode not in MODES:
        raise ValueError(f"unknown L6 union context mode={mode}")
    resolved_device = torch.device(device)
    if resolved_device.type == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("L6 union context requested unavailable CUDA")
    ...
    try:
        model = build_union_context_model(config).to(resolved_device)
        ...
        return LegacyL6UnionContextOutcome(
...
def load_union_context_training_inputs(
    config: LegacyL6UnionContextConfig,
) -> tuple[
    TemporalLadderConfig,
    LegacyL5CachedFeatureView,
    ...
    """Load the frozen T6 actor view and union feature mapping."""
    ...
    _require_equal(
        ...
        "union selection content hash",
    )
    cache = load_union_context_cache(config, base_windows=base.windows)
    _validate_cache_view_alignment(base, cache)
    _validate_selection_for_training(base, cache, selection, config)
    return ladder, base, cache, selection


def union_context_training_git_guard(
    config: LegacyL6UnionContextConfig,
) -> dict[str, Any]:
...
def union_context_implementation_hashes(
    config: LegacyL6UnionContextConfig,
) -> dict[str, str]:
...
def _rename_geometry_surfaces(result: dict[str, Any]) -> None:
    for value in result.values():
        if isinstance(value, pd.DataFrame) and "geometry_mode" in value:
            value.rename(
                columns={"geometry_mode": "union_context_mode"},
                inplace=True,
            )
        elif isinstance(value, dict) and "geometry_mode" in value:
            value["union_context_mode"] = value.pop("geometry_mode")
...
def _validate_config_payload(payload: dict[str, Any]) -> None:
    ...
    _require_exact_keys(payload, required, "L6 union context config")
    ...
    expected_model = {
        "architecture": "cached_visual_union_context_temporal_classifier_v1",
        "feature_control_id": "V1",
        "backbone_name": "resnet18",
        "input_resolution": 224,
        "visual_feature_dim": ACTOR_FEATURE_DIM,
        "union_context_feature_dim": FEATURE_DIM,
        "availability_feature_dim": 1,
        "model_input_dim": MODEL_INPUT_DIM,
        "union_context_modes": list(MODES),
        ...
        "missing_modality_policy": (
            "zero_union_context_and_availability_v1"
        ),
    }
    ...
    if Path(str(output["matrix_gate_filename"])).name != str(
        ...
        raise ValueError("union context matrix gate must be a filename")
