"""Crash-bounded reusable runtime for legacy L6 cached-modality controls."""
...
class CachedModalityRuntimeSpec:
    """Schema, status, and callback adapter for one cached modality."""

    modality_name: str
    modes: tuple[str, ...]
    lineage_scope: str
    short_scope: str
    run_result_schema: str
    run_manifest_schema: str
    artifact_manifest_schema: str
    repeat_gate_schema: str
    matrix_gate_schema: str
    run_audit_schema: str
    failure_schema: str
    checkpoint_schema: str
    checkpoint_manifest_schema: str
    prediction_manifest_schema: str
    environment_schema: str
    pass_training_status: str
    pass_repeat_status: str
    pass_matrix_status: str
    preflight: Callable[[Any, str], dict[str, Any]]
    load_inputs: Callable[[Any], tuple[Any, Any, Any, Any]]
    fit_normalization: Callable[[Any, Any], Any]
    feature_whitelist: Callable[[str], dict[str, Any]]
    train_core: Callable[..., Any]
    git_guard: Callable[[Any], dict[str, Any]]
    implementation_hashes: Callable[[Any], dict[str, str]]


def run_cached_modality_mode(
    spec: CachedModalityRuntimeSpec,
    config: Any,
    *,
    mode: str,
    run_id: str,
    ...
    if _RUN_EXECUTED_IN_PROCESS:
        raise RuntimeError(
            f"L6 {spec.modality_name} permits one run per process"
        )
    if mode not in spec.modes:
        raise ValueError(f"unknown L6 {spec.modality_name} mode={mode}")
    if not _safe_run_id(run_id):
        raise ValueError(f"unsafe L6 {spec.modality_name} run ID={run_id!r}")
    ...
    preflight = spec.preflight(config, mode)
    if not preflight["gpu_launch_authorized"]:
        raise RuntimeError(
            f"L6 {spec.modality_name} preflight failed={preflight['errors']}"
        )
    _RUN_EXECUTED_IN_PROCESS = True
    _, base, cache, selection = spec.load_inputs(config)
    normalization = spec.fit_normalization(cache, selection)
    run_root = config.output_root / mode / run_id
    run_root.mkdir(parents=True, exist_ok=False)
    paths = _run_paths(spec, run_root)
    git_guard = spec.git_guard(config)
    if not git_guard["valid"]:
        raise RuntimeError(
            f"L6 {spec.modality_name} Git guard failed={git_guard['errors']}"
        )
    started_at = _utc_now()
...
def audit_cached_modality_run(
    ...
    *,
    ...
    if resolved.name != _artifact_files(spec)["run_result"]:
        raise ValueError(f"L6 {spec.modality_name} result filename mismatch")
    ...
    errors: list[str] = []
    expected_result = {
        "schema_version": spec.run_result_schema,
        "status": spec.pass_training_status,
        "lineage_scope": spec.lineage_scope,
    ...
    expected_manifest = {
        "schema_version": spec.run_manifest_schema,
    ...
    if result.get("mode") not in spec.modes:
        errors.append(f"unknown_{spec.modality_name}_mode")
    ...
    return {
        "schema_version": spec.run_audit_schema,
        "run_id": result.get("run_id"),
        "mode": result.get("mode"),
...
def audit_cached_modality_repeat_gate(
    spec: CachedModalityRuntimeSpec,
    config: Any,
    *,
    mode: str,
    ...
    """Require deterministic, fresh, non-overlapping short executions."""

    if config.training_scope != spec.short_scope:
        raise ValueError("L6 cached-modality repeat gate requires short scope")
    if mode not in spec.modes:
        raise ValueError(f"unknown L6 {spec.modality_name} mode={mode}")
    primary = audit_cached_modality_run(
        spec,
        config,
        result_path=primary_result_path,
    )
    repeat = audit_cached_modality_run(
        spec,
        config,
    ...
    valid = not errors
    return {
        "schema_version": spec.repeat_gate_schema,
        "status": (
            spec.pass_repeat_status
            if valid
            else spec.pass_repeat_status.replace("PASS_", "FAIL_", 1)
        ),
        "lineage_scope": spec.lineage_scope,
        "training_scope": config.training_scope,
        "mode": mode,
...
def write_cached_modality_repeat_gate(
    spec: CachedModalityRuntimeSpec,
    config: Any,
    *,
    mode: str,
    ...
    gate = audit_cached_modality_repeat_gate(
        spec,
        config,
        mode=mode,
...
def audit_cached_modality_short_matrix(
    spec: CachedModalityRuntimeSpec,
    config: Any,
    *,
    repeat_gate_paths: dict[str, Path],
) -> dict[str, Any]:
    """Close all parameter-matched controls before full expansion."""

    if config.training_scope != spec.short_scope:
        raise ValueError("L6 cached-modality matrix requires short scope")
    if set(repeat_gate_paths) != set(spec.modes):
        raise ValueError("L6 cached-modality matrix mode set drift")
    errors: list[str] = []
    ...
    for mode in spec.modes:
        ...
        expected = {
            "schema_version": spec.repeat_gate_schema,
            "status": spec.pass_repeat_status,
            "lineage_scope": spec.lineage_scope,
            "training_scope": spec.short_scope,
            "mode": mode,
            "short_config_sha256": config.sha256,
    ...
    valid = not errors
    return {
        "schema_version": spec.matrix_gate_schema,
        "status": (
            spec.pass_matrix_status
            if valid
            else spec.pass_matrix_status.replace("PASS_", "FAIL_", 1)
        ),
        "lineage_scope": spec.lineage_scope,
        "training_scope": spec.short_scope,
        "short_config_sha256": config.sha256,
...
def write_cached_modality_short_matrix(
    spec: CachedModalityRuntimeSpec,
    config: Any,
    *,
    repeat_gate_paths: dict[str, Path],
    ...
    matrix = audit_cached_modality_short_matrix(
        spec,
        config,
...
def _execute_cuda(
    ...
    *,
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(f"L6 {spec.modality_name} requires CUDA")
    ...
    if outcome is None:
        raise RuntimeError(
            f"L6 {spec.modality_name} CUDA execution produced no outcome"
        )
    if post_allocated != 0 or post_reserved != 0:
        raise RuntimeError(
            f"L6 {spec.modality_name} CUDA cleanup failed "
...
def _finalize_run(
    ...
    *,
    ...
    _write_json_exclusive(
        paths["environment"],
        _environment_payload(spec, execution),
    )
...
def _write_outcome(
    ...
    *,
    ...
    frames = {
        ...
        "window_predictions": outcome.window_predictions,
        "native_predictions": outcome.native_predictions,
        "validation_per_class": outcome.per_class_metrics,
        "validation_confusion": outcome.confusion,
        "confusion_groups": outcome.confusion_groups,
        "missing_window_predictions": outcome.missing_window_predictions,
        "missing_native_predictions": outcome.missing_native_predictions,
        "missing_confusion_groups": outcome.missing_confusion_groups,
    }
    for name, frame in frames.items():
        _write_dataframe_exclusive(paths[name], frame)
    ...
    checkpoint_manifest = {
        "schema_version": spec.checkpoint_manifest_schema,
        "mode": mode,
        "config_sha256": config.sha256,
    ...
    prediction_manifest = {
        "schema_version": spec.prediction_manifest_schema,
        "mode": mode,
        "config_sha256": config.sha256,
...
def _success_result(
    ...
    *,
    ...
    return {
        ...
        "config_sha256": config.sha256,
        "implementation_hashes": spec.implementation_hashes(config),
...
def _failure_result(
    ...
    *,
    ...
    return {
        "schema_version": spec.run_result_schema,
        "status": spec.pass_training_status.replace("PASS_", "FAIL_", 1),
        "run_id": run_id,
        "process_id": os.getpid(),
        "mode": mode,
        "training_scope": config.training_scope,
...
def _failed_execution(config: Any, error: Exception) -> dict[str, Any]:
...
def _failure_packet(
    ...
    *,
    ...
    return {
        "schema_version": spec.failure_schema,
        "run_id": run_id,
        "mode": mode,
        "process_id": os.getpid(),
...
def _environment_payload(
    ...
    return {
        "schema_version": spec.environment_schema,
        "process_id": os.getpid(),
        "python": sys.version,
...
def _build_artifact_manifest(
    ...
    *,
    ...
    return {
        "schema_version": spec.artifact_manifest_schema,
        "run_id": run_id,
        "mode": mode,
        "status": status,
        "artifacts": artifacts,
...
def _validate_run_path_lengths(
    ...
    *,
    ...
    if limit_applies and maximum > MAX_WINDOWS_ARTIFACT_PATH_CHARS:
        raise ValueError(
            f"L6 {spec.modality_name} Windows artifact path too long "
...
def _artifact_files(spec: CachedModalityRuntimeSpec) -> dict[str, str]:
    return {
        "environment": "environment.json",
        "preflight": "preflight.json",
        "selection_manifest": "training_selection_manifest.csv",
        "selection_audit": "training_selection_audit.json",
        "normalization": f"{spec.modality_name}_normalization.json",
        ...
        "missing_validation_metrics": (
            "missing_modality_validation_metrics.json"
        ),
...
def _run_paths(
    ...
    paths = {
        name: root / filename
        for name, filename in _artifact_files(spec).items()
    }
    paths.update(
        {
...
def _object(value: object, name: str) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise ValueError(f"L6 cached-modality {name} must be an object")
    return value
