...
class M2BranchEmbeddings:
    """The four branch tensors exposed by the locked M2 architecture."""

    actor128: torch.Tensor
    structured128: torch.Tensor
    interaction64: torch.Tensor
    union128: torch.Tensor

    def concatenate(self) -> torch.Tensor:
        """Return the unchanged M2 448D late-fusion representation."""
...
class MultimodalFusionClassifier(nn.Module):
    ...
    def _encode_actor_branch(
        ...
        *,
        ...
        """Encode actor crop sequence. Subclasses may override this seam."""
    ...
    def encode_fused(
        ...
        *,
        ...
        partner_quality_mask: torch.Tensor | None = None,
        partner_time_delta: torch.Tensor | None = None,
        return_m2_branches: bool = False,
    ) -> torch.Tensor | M2BranchEmbeddings:
        """Return the shared late-fusion embedding before classification heads."""

        embeddings: list[torch.Tensor] = []
        batch_size: int | None = None
        if self.image_encoder is not None:
            image_embedding = self._encode_actor_branch(
                image=image,
                spatial_features=spatial_features,
                length_mask=(
                    image_length_mask
        ...
        if return_m2_branches:
            expected_dims = (128, 128, 64, 128)
            actual_dims = tuple(int(value.shape[-1]) for value in embeddings)
            if self.partner_encoder is not None or actual_dims != expected_dims:
                raise ValueError(
                    "M2 branch exposure requires actor128, structured128, "
                    "interaction64, and union128"
                )
            return M2BranchEmbeddings(
                actor128=embeddings[0],
                structured128=embeddings[1],
                interaction64=embeddings[2],
                union128=embeddings[3],
            )
