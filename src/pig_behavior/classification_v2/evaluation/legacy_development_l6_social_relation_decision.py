"""Paired native-unit decision for the legacy L6 numeric-social matrix."""
...
def evaluate_social_relation_short_decision(
    config_path: Path,
    *,
    project_root: Path,
) -> dict[str, Any]:
    """Audit three packets and apply the predeclared paired decision."""
    ...
    for name, value in _object(
        ...
        _validate_bound_file(
            ...
            spec["sha256"],
            f"social decision implementation.{name}",
        )
    ...
    _validate_bound_file(
        ...
        "social relation short training config",
    )
    training_config = load_social_relation_training_config(training_path)
    if training_config.training_scope != SHORT_SCOPE:
        raise ValueError("L6 social decision requires short training scope")
    ...
    comparisons = {
        "social_relation_vs_parameter_matched_zero": _compare_packets(
            packets["social_relation"],
            packets["parameter_matched_zero"],
            contract=paired_contract,
        ),
        "social_relation_vs_availability_only": _compare_packets(
            packets["social_relation"],
    ...
    decision = make_social_relation_decision(
        comparisons,
    ...
    return {
        ...
        "status": (
            "PASS_LEGACY_DEVELOPMENT_L6_SOCIAL_RELATION_SHORT_DECISION"
            if valid
            else "FAIL_LEGACY_DEVELOPMENT_L6_SOCIAL_RELATION_SHORT_DECISION"
        ),
...
def make_social_relation_decision(
    comparisons: dict[str, dict[str, Any]],
    *,
    contract: dict[str, Any],
) -> dict[str, Any]:
    """Apply every promotion criterion and preserve valid rejection."""

    zero = comparisons["social_relation_vs_parameter_matched_zero"]
    availability = comparisons["social_relation_vs_availability_only"]
    ...
    zero_delta = float(
        zero["delta_candidate_minus_baseline"][
    ...
    criteria = {
        "social_gain_vs_zero_meets_margin": zero_delta >= minimum_gain,
        "social_gain_vs_availability_meets_margin": (
            availability_delta >= minimum_gain
        ),
        "social_vs_zero_cluster_ci_low_positive": (
            float(zero["video_cluster_bootstrap"]["ci_low"]) > 0.0
        ),
        "social_vs_availability_cluster_ci_low_positive": (
            float(availability["video_cluster_bootstrap"]["ci_low"]) > 0.0
        ),
        "social_nll_improves_vs_zero": (
    ...
    return {
        "decision": (
            ...
            if authorized
            else "DO_NOT_EXPAND_SOCIAL_RELATION_FROM_CURRENT_SHORT_EVIDENCE"
        ),
        "criteria": criteria,
        "thresholds": dict(contract),
        "full_social_relation_expansion_authorized": authorized,
        ...
        "next_action": (
            "prepare_hash_bound_full_social_relation_config"
            if authorized
            else "retain_parameter_matched_zero_and_continue_l6_top_k_partner"
        ),
    }


def write_social_relation_short_decision(
    config_path: Path,
    *,
    project_root: Path,
) -> tuple[Path, dict[str, Any]]:
    payload = evaluate_social_relation_short_decision(
        config_path,
...
def _load_matrix(
    root: Path,
    value: object,
    training_config: LegacyL6SocialRelationConfig,
) -> dict[str, Any]:
    ...
    expected = {
        ...
        "status": "PASS_LEGACY_DEVELOPMENT_L6_SOCIAL_RELATION_SHORT_MATRIX",
        "lineage_scope": LINEAGE_SCOPE,
        "training_scope": SHORT_SCOPE,
...
def _load_packet(
    ...
    *,
    ...
    audit = audit_social_relation_run(training_config, result_path=result_path)
    if not audit["valid"]:
        raise ValueError(f"L6 social relation run audit failed mode={mode}")
    ...
    _require_mapping(
        repeat,
        {
            ...
            "status": "PASS_LEGACY_DEVELOPMENT_L6_SOCIAL_RELATION_REPEAT",
            "mode": mode,
            "short_config_sha256": training_config.sha256,
...
def _validate_common_universe(
    ...
    for mode in MODES:
        ...
        if set(frame["social_relation_mode"].astype(str)) != {mode}:
...
def _validate_config(config: dict[str, Any]) -> None:
    ...
    _require_exact_keys(config, required, "social relation decision config")
    ...
    _require_exact_keys(
        implementation,
        {"evaluator", "paired_engine", "training_adapter", "runtime_adapter"},
        "implementation",
    )
...
def _git_guard(root: Path, guard: dict[str, Any]) -> dict[str, Any]:
    ...
    observed = sorted(line[3:].strip().replace("\\", "/") for line in entries)
    allowed = sorted(
        str(value).replace("\\", "/") for value in guard["allowed_dirty_paths"]
    )
