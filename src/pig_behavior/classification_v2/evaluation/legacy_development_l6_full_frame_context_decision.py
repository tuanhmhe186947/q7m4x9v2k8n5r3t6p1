"""Paired native/video-cluster decision for legacy L6 full-frame context."""
...
from pig_behavior.classification_v2.training import (
    legacy_development_l6_full_frame_context as full_frame_core,
)
from pig_behavior.classification_v2.training import (
...
LINEAGE_SCOPE = full_frame_core.LINEAGE_SCOPE
MODES = full_frame_core.MODES
SHORT_SCOPE = full_frame_core.SHORT_SCOPE
LegacyL6FullFrameContextConfig = (
    full_frame_core.LegacyL6FullFrameContextConfig
)
load_full_frame_context_training_config = (
    full_frame_core.load_full_frame_context_training_config
)
MATRIX_GATE_SCHEMA = full_frame_runtime.MATRIX_GATE_SCHEMA
REPEAT_GATE_SCHEMA = full_frame_runtime.REPEAT_GATE_SCHEMA
CONFIG_SCHEMA = (
    "classification_v2.legacy_development_l6."
    "full_frame_context_decision_config.v1"
)
RESULT_SCHEMA = (
    "classification_v2.legacy_development_l6."
    "full_frame_context_decision.v1"
)
EXPECTED_NATIVE_UNITS = 245
EXPECTED_VIDEO_CLUSTERS = 33
EXPECTED_PARAMETER_COUNT = full_frame_core.EXPECTED_PARAMETER_COUNT


def evaluate_full_frame_context_short_decision(
    config_path: Path,
    *,
    project_root: Path,
) -> dict[str, Any]:
    """Audit the full-frame short matrix and apply the paired promotion gate."""

    root = project_root.resolve()
    resolved_config = config_path.resolve()
    config = _read_json(resolved_config)
    _validate_config(config)
    for name, value in _object(config["implementation"], "implementation").items():
        spec = _object(value, f"implementation.{name}")
        _validate_bound_file(
            _resolve_inside(root, spec["path"]),
            spec["sha256"],
            f"full-frame decision implementation.{name}",
        )
    ...
    _validate_bound_file(
        ...
        "full-frame short training config",
    )
    training_config = load_full_frame_context_training_config(training_path)
    if training_config.training_scope != SHORT_SCOPE:
        raise ValueError("full-frame decision requires short training scope")
    ...
    comparisons = {
        "full_frame_context_vs_parameter_matched_zero": _compare_packets(
            packets["full_frame_context"],
            packets["parameter_matched_zero"],
            contract=paired_contract,
        ),
        "full_frame_context_vs_availability_only": _compare_packets(
            packets["full_frame_context"],
    ...
    decision = make_full_frame_context_decision(
        comparisons,
    ...
    return {
        ...
        "status": (
            "PASS_LEGACY_DEVELOPMENT_L6_FULL_FRAME_CONTEXT_SHORT_DECISION"
            if not git_guard["errors"]
            else "FAIL_LEGACY_DEVELOPMENT_L6_FULL_FRAME_CONTEXT_SHORT_DECISION"
        ),
...
def make_full_frame_context_decision(
    comparisons: dict[str, dict[str, Any]],
    *,
    contract: dict[str, Any],
) -> dict[str, Any]:
    """Apply the predeclared one-family full-frame promotion criteria."""

    zero = comparisons["full_frame_context_vs_parameter_matched_zero"]
    availability = comparisons["full_frame_context_vs_availability_only"]
    ...
    criteria = {
        "full_frame_gain_vs_zero_meets_margin": (
            zero_delta >= minimum_gain
        ),
        "full_frame_gain_vs_availability_meets_margin": (
            availability_delta >= minimum_gain
        ),
        "full_frame_vs_zero_cluster_ci_low_positive": (
            float(zero["video_cluster_bootstrap"]["ci_low"]) > 0.0
        ),
        "full_frame_vs_availability_cluster_ci_low_positive": (
            float(availability["video_cluster_bootstrap"]["ci_low"]) > 0.0
        ),
        "full_frame_nll_improves_vs_zero": (
    ...
    return {
        "decision": (
            ...
            if authorized
            else "DO_NOT_EXPAND_FULL_FRAME_CONTEXT_FROM_CURRENT_SHORT_EVIDENCE"
        ),
        "criteria": criteria,
        "thresholds": copy.deepcopy(contract),
        "full_frame_context_expansion_authorized": authorized,
        ...
        "full_frame_context_values_in_model_x": True,
        "source_probe_status": "NOT_ESTIMABLE_SINGLE_LEGACY_SOURCE",
        "next_action": (
            "prepare_hash_bound_full_frame_context_confirmation"
            if authorized
            else "continue_l7_imbalance_from_parameter_matched_zero"
        ),
    }


def write_full_frame_context_short_decision(
    config_path: Path,
    *,
    project_root: Path,
) -> tuple[Path, dict[str, Any]]:
    payload = evaluate_full_frame_context_short_decision(
        config_path,
...
def _load_matrix(
    root: Path,
    value: object,
    training_config: LegacyL6FullFrameContextConfig,
) -> dict[str, Any]:
...
def _load_packet(
    ...
    *,
    ...
    audit = full_frame_runtime.audit_full_frame_context_run(
        training_config,
        result_path=result_path,
    )
    if not audit["valid"]:
        raise ValueError(f"full-frame run audit failed mode={mode}")
...
def _validate_common_universe(
    ...
    for mode in MODES:
        ...
        if set(frame["full_frame_context_mode"].astype(str)) != {mode}:
...
def _validate_config(config: dict[str, Any]) -> None:
    ...
    _require_exact_keys(config, required, "full-frame decision config")
