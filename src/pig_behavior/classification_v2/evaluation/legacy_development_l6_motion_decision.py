"""Paired native-unit/video-cluster decision for the legacy L6 motion gate."""
...
CONFIG_SCHEMA = (
    "classification_v2.legacy_development_l6.motion_decision_config.v1"
)
RESULT_SCHEMA = (
    "classification_v2.legacy_development_l6.motion_decision.v1"
)
...
def evaluate_motion_short_decision(
    config_path: Path,
    *,
    project_root: Path,
) -> dict[str, Any]:
    """Audit the three short packets and produce a paired decision."""
    ...
    _validate_bound_file(
        ...
        "motion decision implementation",
    )
    ...
    _validate_bound_file(
        ...
        "motion short training config",
    )
    training_config = load_motion_training_config(training_path)
    if training_config.training_scope != SHORT_SCOPE:
        raise ValueError("L6 motion decision requires short training scope")
    ...
    comparisons = {
        "motion_vs_parameter_matched_zero": _compare_packets(
            packets["motion"],
            packets["parameter_matched_zero"],
            contract=contract,
        ),
        "motion_vs_availability_only": _compare_packets(
            packets["motion"],
    ...
    return {
        ...
        else "FAIL_LEGACY_DEVELOPMENT_L6_MOTION_SHORT_DECISION",
...
def make_motion_decision(
    comparisons: dict[str, dict[str, Any]],
    *,
    contract: dict[str, Any],
) -> dict[str, Any]:
    """Apply predeclared promotion criteria without hiding rejection evidence."""

    zero = comparisons["motion_vs_parameter_matched_zero"]
    availability = comparisons["motion_vs_availability_only"]
    ...
    criteria = {
        "motion_gain_vs_zero_meets_margin": zero_delta >= minimum_gain,
        "motion_gain_vs_availability_meets_margin": (
            availability_delta >= minimum_gain
        ),
        "motion_vs_zero_cluster_ci_low_positive": (
            float(zero["video_cluster_bootstrap"]["ci_low"]) > 0.0
        ),
        "motion_vs_availability_cluster_ci_low_positive": (
            float(availability["video_cluster_bootstrap"]["ci_low"]) > 0.0
        ),
        "motion_nll_improves_vs_zero": (
    ...
    return {
        "decision": (
            ...
            if authorized
            else "DO_NOT_EXPAND_MOTION_FROM_CURRENT_SHORT_EVIDENCE"
        ),
        "criteria": criteria,
        "thresholds": dict(contract),
        "full_motion_expansion_authorized": authorized,
        ...
        "next_action": (
            "prepare_hash_bound_full_motion_config"
            if authorized
            else "retain_parameter_matched_zero_and_stop_motion_expansion"
        ),
    }


def write_motion_short_decision(
    config_path: Path,
    *,
    project_root: Path,
) -> tuple[Path, dict[str, Any]]:
    payload = evaluate_motion_short_decision(
        config_path,
...
def _load_matrix(
    root: Path,
    value: object,
    training_config: LegacyL6MotionConfig,
) -> dict[str, Any]:
    ...
    expected = {
        ...
        "status": "PASS_LEGACY_DEVELOPMENT_L6_MOTION_SHORT_MATRIX",
        "lineage_scope": LINEAGE_SCOPE,
        "training_scope": SHORT_SCOPE,
...
def _load_packet(
    ...
    *,
    ...
    audit = audit_motion_run(training_config, result_path=result_path)
    if not audit["valid"]:
        raise ValueError(f"L6 motion run audit failed mode={mode}")
    ...
    _require_mapping(
        repeat,
        {
            ...
            "status": "PASS_LEGACY_DEVELOPMENT_L6_MOTION_REPEAT",
            "mode": mode,
            "short_config_sha256": training_config.sha256,
...
def _validate_common_universe(
    ...
    for mode in MODES:
        ...
        if set(frame["motion_mode"].astype(str)) != {mode}:
...
def _validate_config(config: dict[str, Any]) -> None:
    ...
    _require_exact_keys(config, required, "motion decision config")
