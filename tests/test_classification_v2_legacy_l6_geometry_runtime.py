...
def _short_config(tmp_path: Path) -> LegacyL6GeometryConfig:
...
def test_runtime_run_id_and_non_overlap_guards() -> None:
    assert _safe_run_id("geometry_repeat-01")
    ...
    overlapping["started_at_utc"] = (start + timedelta(seconds=30)).isoformat()
...
def test_short_matrix_requires_six_distinct_processes(tmp_path: Path) -> None:
    ...
    matrix = audit_geometry_short_matrix(config, repeat_gate_paths=paths)
    ...
    failed = audit_geometry_short_matrix(config, repeat_gate_paths=paths)
