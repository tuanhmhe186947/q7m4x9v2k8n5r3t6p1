...
Notebook quick start:
    ...
    cfg = TrackingConfig(
        ...
        mask_path=Path("data/annotations/mask.png"),
    )
...
Command line:
    ...
    python notebooks/01_data_preparation/track_video_ids_for_annotation.py \
...
DEFAULT_MASK_PATH = PROJECT_ROOT / "data" / "annotations" / "mask.png"
...
def suppress_duplicate_detections(
    ...
    for det in detections:
        if all(
            bbox_iou(det.box, other.box) < cfg.duplicate_iou_threshold
            for other in kept
        ):
            kept.append(det)
    return kept
...
def initialize_tracks(
    ...
    selected = spatial_sort_detections(init_detections)[: cfg.expected_pigs]
    median_box = None
    if selected:
    ...
    for fixed_id, det in enumerate(selected, start=1):
        nearest_idx = min(
            range(len(anchors)),
            key=lambda idx: center_distance_norm(anchors[idx], det.box, width, height),
        )
        used_anchor_idx.add(nearest_idx)
        track = FixedTrack(fixed_id=fixed_id, last_box=det.box.copy())
        track.update_detected(det, width, height, cfg)
        tracks[fixed_id] = track

    remaining_anchors = [
        anchor for idx, anchor in enumerate(anchors) if idx not in used_anchor_idx
    ]
    for fixed_id in range(len(selected) + 1, cfg.expected_pigs + 1):
        if remaining_anchors:
            box = remaining_anchors.pop(0)
        else:
            box = anchors[(fixed_id - 1) % len(anchors)]
        tracks[fixed_id] = FixedTrack(fixed_id=fixed_id, last_box=box.copy())
...
def low_conf_detection_is_plausible(
    ...
    iou_score = bbox_iou(predicted, det.box)
    if iou_score >= cfg.low_conf_min_iou:
        return True

    center_norm = center_distance_norm(predicted, det.box, width, height)
    allowed_norm = cfg.low_conf_max_center_jump + min(track.missed, 30) * 0.004
    if center_norm <= allowed_norm:
        return True

    pred_cx, pred_cy = bbox_center(predicted)
    det_cx, det_cy = bbox_center(det.box)
    pred_w, pred_h = bbox_size(predicted)
...
def main(argv: list[str] | None = None) -> int:
    ...
    cfg = TrackingConfig(
        ...
        mask_path=None if args.no_mask else args.mask,
