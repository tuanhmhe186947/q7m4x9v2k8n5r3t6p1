...
print("=== 1. BASELINE PILOT (batch=32, BF16, eager, num_workers=0) ===", flush=True)
...
print(f"BASELINE: {res_baseline['samples_per_sec']:.1f} samples/s, p50={res_baseline['p50_step_s']*1000:.1f}ms, Peak VRAM={res_baseline['peak_vram_gb']:.2f}GB", flush=True)
...
for bs in [16, 32, 64, 128]:
    try:
        r = run_benchmark(batch_size=bs, num_workers=0, amp_dtype=torch.bfloat16, warmup_steps=10, measure_steps=30)
        batch_results[str(bs)] = r
        print(f"BS={bs}: {r['samples_per_sec']:.1f} samples/s ({r['steps_per_sec']:.1f} steps/s), p50={r['p50_step_s']*1000:.1f}ms, Peak VRAM={r['peak_vram_gb']:.2f}GB", flush=True)
...
print(f"=== 3. AMP COMPARISON on BS={best_bs} ===", flush=True)
res_bf16 = run_benchmark(batch_size=best_bs, num_workers=0, amp_dtype=torch.bfloat16, warmup_steps=10, measure_steps=30)
res_fp16 = run_benchmark(batch_size=best_bs, num_workers=0, amp_dtype=torch.float16, warmup_steps=10, measure_steps=30)
results["amp"] = {"bf16": res_bf16, "fp16": res_fp16}
print(f"BF16: {res_bf16['samples_per_sec']:.1f} samples/s, Peak VRAM={res_bf16['peak_vram_gb']:.2f}GB", flush=True)
print(f"FP16: {res_fp16['samples_per_sec']:.1f} samples/s, Peak VRAM={res_fp16['peak_vram_gb']:.2f}GB", flush=True)
...
# 4. WORKER TEST (num_workers=0 vs 2 on best_bs)
print(f"=== 4. WORKER TEST on BS={best_bs} ===", flush=True)
worker_results = {}
for nw in [0, 2]:
    try:
        r = run_benchmark(batch_size=best_bs, num_workers=nw, amp_dtype=selected_amp, warmup_steps=10, measure_steps=30)
        worker_results[str(nw)] = r
        print(f"NW={nw}: {r['samples_per_sec']:.1f} samples/s, Data Wait: {r['data_wait_fraction']*100:.1f}%", flush=True)
    except Exception as e:
        print(f"NW={nw} failed: {e}", flush=True)
...
print(f"=== 5. CHANNELS LAST TEST on BS={best_bs} ===", flush=True)
res_contig = run_benchmark(batch_size=best_bs, num_workers=best_nw, amp_dtype=selected_amp, channels_last=False, warmup_steps=10, measure_steps=30)
try:
    res_cl = run_benchmark(batch_size=best_bs, num_workers=best_nw, amp_dtype=selected_amp, channels_last=True, warmup_steps=10, measure_steps=30)
    results["channels_last"] = {"contiguous": res_contig, "channels_last": res_cl}
    print(f"Contiguous: {res_contig['samples_per_sec']:.1f} samples/s", flush=True)
    print(f"Channels Last: {res_cl['samples_per_sec']:.1f} samples/s", flush=True)
...
# 6. TORCH.COMPILE TEST (OPTIONAL)
print(f"=== 6. TORCH.COMPILE POLICY ===", flush=True)
print("Evaluating eager vs compile: eager selected to avoid compile overhead and dynamic batch shape issues.", flush=True)
use_comp = False
results["compile"] = {"tested": False, "reason": "Eager chosen per contract (recovering compile overhead not guaranteed)"}

# 7. FINAL SELECTED PROFILE VERIFICATION (50 steps)
print(f"=== 7. FINAL SELECTED PROFILE VERIFICATION ===", flush=True)
...
print(f"FINAL PROFILE VERIFIED: {final_prof['samples_per_sec']:.1f} samples/s, p50={final_prof['p50_step_s']*1000:.1f}ms, Peak VRAM={final_prof['peak_vram_gb']:.2f}GB", flush=True)
...
cmd = f"python3 -c \"import base64; open('/teamspace/studios/this_studio/bench_l4.py', 'w').write(base64.b64decode('{b64_code}').decode('utf-8'))\""
res = studio.run(cmd)
print("Uploaded bench_l4.py to Studio.")

# Run bench_l4.py
res = studio.run("python3 /teamspace/studios/this_studio/bench_l4.py")
