...
res_bf16 = run_benchmark(batch_size=128, num_workers=0, amp_dtype=torch.bfloat16)
res_fp16 = run_benchmark(batch_size=128, num_workers=0, amp_dtype=torch.float16)
...
res_contig = res_bf16
try:
    res_cl = run_benchmark(batch_size=128, num_workers=0, amp_dtype=selected_amp, channels_last=True)
...
# 7. FINAL SELECTED PROFILE VERIFICATION (15 steps)
...
cmd = f"python3 -c \"import base64; open('/teamspace/studios/this_studio/bench_fast.py', 'w').write(base64.b64decode('{b64_code}').decode('utf-8'))\""
studio.run(cmd)
print("Uploaded bench_fast.py to Studio.")

# Run bench_fast.py
res = studio.run("python3 /teamspace/studios/this_studio/bench_fast.py")
