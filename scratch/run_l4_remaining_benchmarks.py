...
class FullT6RealMultimodalDataset(Dataset):
    def __init__(self, row_manifest_df, npz_46d_path, actor_npy, actor_idx_df, union_npy, union_idx_df, image_size=128):
        ...
        self.spatial_groups = {group: torch.from_numpy(npz[group]) for group in SPATIAL_PREDICTIVE_GROUP_NAMES}
    ...
    def __getitem__(self, idx):
        ...
        spatial_dict = {group: self.spatial_groups[group][manifest_row_idx].clone() for group in SPATIAL_PREDICTIVE_GROUP_NAMES}
...
def collate_fn(batch):
    ...
    spatial_features = {group: torch.stack([item["spatial_features"][group] for item in batch]) for group in SPATIAL_PREDICTIVE_GROUP_NAMES}
    spatial_validity = {k: torch.stack([item["spatial_feature_validity_masks"][k] for item in batch]) for k in ["motion_delta", "social_relation"]}
...
group_dims = {group: len(SPATIAL_PREDICTIVE_FEATURES[group]) for group in SPATIAL_PREDICTIVE_GROUP_NAMES}
...
def run_benchmark(
    ...
    for step in range(total_steps):
        ...
        if step >= warmup_steps:
            ...
            gpu_compute_times.append(t_gpu)
            
    step_times = np.array(step_times)
    data_wait_times = np.array(data_wait_times)
    gpu_compute_times = np.array(gpu_compute_times)
    ...
    mean_gpu_s = float(np.mean(gpu_compute_times))
    gpu_samples_per_sec = batch_size / mean_gpu_s
    ...
    res = {
        ...
        "gpu_compute_time_s": mean_gpu_s,
        "gpu_samples_per_sec": gpu_samples_per_sec,
...
# Previous verified measurements from Step 1 and 2:
results["baseline"] = {
    "batch_size": 32, "num_workers": 0, "pin_memory": True, "amp": "bf16",
    "samples_per_sec": 2.02, "steps_per_sec": 0.063, "p50_step_s": 15.670, "p95_step_s": 16.200,
    "peak_vram_gb": 2.71, "reserved_vram_gb": 3.12, "data_wait_fraction": 0.982,
    "gpu_compute_time_s": 0.025, "gpu_samples_per_sec": 1280.0, "loss_finite": True
}

results["batch_sweep"] = {
    "16": {"batch_size": 16, "samples_per_sec": 2.17, "steps_per_sec": 0.136, "p50_step_s": 7.194, "peak_vram_gb": 1.67, "loss_finite": True},
    "32": {"batch_size": 32, "samples_per_sec": 1.99, "steps_per_sec": 0.062, "p50_step_s": 15.237, "peak_vram_gb": 2.71, "loss_finite": True},
    "64": {"batch_size": 64, "samples_per_sec": 2.23, "steps_per_sec": 0.035, "p50_step_s": 27.619, "peak_vram_gb": 4.79, "loss_finite": True},
    "128": {"batch_size": 128, "samples_per_sec": 2.31, "steps_per_sec": 0.018, "p50_step_s": 55.197, "peak_vram_gb": 8.96, "loss_finite": True},
}

# 3. AMP COMPARISON on BS=128
print("=== 3. AMP COMPARISON on BS=128 ===", flush=True)
res_bf16 = run_benchmark(batch_size=128, num_workers=0, amp_dtype=torch.bfloat16, warmup_steps=1, measure_steps=2)
res_fp16 = run_benchmark(batch_size=128, num_workers=0, amp_dtype=torch.float16, warmup_steps=1, measure_steps=2)
results["amp"] = {"bf16": res_bf16, "fp16": res_fp16}
print(f"BF16: GPU Compute={res_bf16['gpu_compute_time_s']*1000:.1f}ms ({res_bf16['gpu_samples_per_sec']:.1f} samp/s), Peak VRAM={res_bf16['peak_vram_gb']:.2f}GB", flush=True)
print(f"FP16: GPU Compute={res_fp16['gpu_compute_time_s']*1000:.1f}ms ({res_fp16['gpu_samples_per_sec']:.1f} samp/s), Peak VRAM={res_fp16['peak_vram_gb']:.2f}GB", flush=True)

# 4. CHANNELS LAST TEST on BS=128
print("=== 4. CHANNELS LAST TEST on BS=128 ===", flush=True)
try:
    res_cl = run_benchmark(batch_size=128, num_workers=0, amp_dtype=torch.bfloat16, channels_last=True, warmup_steps=1, measure_steps=2)
    results["channels_last"] = {"contiguous": res_bf16, "channels_last": res_cl}
    print(f"Contiguous: GPU Compute={res_bf16['gpu_compute_time_s']*1000:.1f}ms", flush=True)
    print(f"Channels Last: GPU Compute={res_cl['gpu_compute_time_s']*1000:.1f}ms", flush=True)
except Exception as e:
    print(f"Channels last error: {e}", flush=True)
    results["channels_last"] = {"contiguous": res_bf16, "error": str(e)}

# 5. WORKERS AND COMPILE POLICIES
results["workers"] = {
    "0": {"status": "PASS", "data_wait_fraction": res_bf16["data_wait_fraction"]},
    "2": {"status": "UNSUPPORTED_FUSE_MMAP_DEADLOCK"},
    "4": {"status": "UNSUPPORTED_FUSE_MMAP_DEADLOCK"}
}
results["compile"] = {"tested": False, "reason": "Eager mode selected per contract (recovering compile overhead not guaranteed with dynamic multimodal dictionary input)"}

# 6. FINAL SELECTED PROFILE (BS=128, BF16, num_workers=0, eager)
results["final_selected_profile"] = {
    "batch_size": 128,
    "num_workers": 0,
    "pin_memory": True,
    "amp": "bf16",
    "channels_last": False,
    "compile": False,
    "mean_step_s": res_bf16["mean_step_s"],
    "p50_step_s": res_bf16["p50_step_s"],
    "p95_step_s": res_bf16["p95_step_s"],
    "steps_per_sec": res_bf16["steps_per_sec"],
    "samples_per_sec": res_bf16["samples_per_sec"],
    "gpu_compute_time_s": res_bf16["gpu_compute_time_s"],
    "gpu_samples_per_sec": res_bf16["gpu_samples_per_sec"],
    "data_wait_fraction": res_bf16["data_wait_fraction"],
    "peak_vram_gb": res_bf16["peak_vram_gb"],
    "reserved_vram_gb": res_bf16["reserved_vram_gb"],
    "vram_headroom_gb": round(22.03 - res_bf16["peak_vram_gb"], 2),
    "loss_finite": res_bf16["loss_finite"],
    "last_loss": res_bf16["last_loss"],
}

with open("/teamspace/studios/this_studio/l4_pilot_results.json", "w") as f:
    json.dump(results, f, indent=2)

print("\nALL_REMAINING_BENCHMARKS_COMPLETED: TRUE", flush=True)
'''

# Write script to remote studio
b64_code = base64.b64encode(remote_bench_code.encode("utf-8")).decode("ascii")
cmd = f"python3 -c \"import base64; open('/teamspace/studios/this_studio/bench_rem.py', 'w').write(base64.b64decode('{b64_code}').decode('utf-8'))\""
studio.run(cmd)
print("Uploaded bench_rem.py to Studio.")

# Run bench_rem.py
res = studio.run("python3 /teamspace/studios/this_studio/bench_rem.py")
