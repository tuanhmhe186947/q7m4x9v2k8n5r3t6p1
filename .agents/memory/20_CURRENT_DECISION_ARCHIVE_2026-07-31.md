# Current Decision Archive

## Memory lifetime contract (2026-07-31)

- User-approved policy: short memory lasts only for the current session/day.
- Short memory expires at the next `Asia/Saigon` calendar day.
- Unfinished cross-day state moves to medium; resolved session detail is dropped.
- Long memory is restricted to charter, accepted contracts, stable facts, and
  complete project-wide information.
- The pre-migration short history is preserved at
  `17_PROJECT_MEMORY_SHORT_ARCHIVE_2026-07-31.md`.
- State: `ACCEPTED` after governance validation, focused regression, and an
  independent read-only forward test.
- Current implementation authority is `00`, `03`, `08`, and
  `.agents/skills/project-state-steward/SKILL.md`.

## Automatic project-state stewardship (2026-07-31)

- Project-local authority:
  `.agents/skills/project-state-steward/SKILL.md`.
- Root `AGENTS.md` requires the skill before final handoff after material
  implementation changes or a validated durable correction.
- Learning admission requires root cause, validated correction, evidence,
  reuse conditions, and non-reuse boundaries.
- Cleanup is evidence-gated. Unknown, user-owned, scientific, or lineage-bearing
  paths remain protected and are recorded in `10_REPO_HYGIENE.md`.
- Automatic `continuous-learning-v2` observer hooks remain inactive.

## Skill portfolio balance (2026-07-31)

- The project has one maintenance-session usage record for
  `agent-self-evaluation`; the broader reasoning and synthesis queue still has
  no production-task usage evidence in `11_SKILL_PORTFOLIO.md`.
- Nontrivial architecture, debugging, evaluation, synthesis, and data-contract
  work must select at least one reasoning or verification skill.
- Code-skill maintenance is triggered by corrections, repeated failures, tool or
  dependency changes, validator failure, or 30 days without review.
