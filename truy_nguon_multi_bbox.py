...
DEFAULT_BEHAVIOR_CSV = "/content/behavior_with_feats_rectROI.csv"
...
DEFAULT_OUT_CENTER = "/content/old_burst_center_keyframes_combined.csv"
DEFAULT_OUT_ALL_BBOX = "/content/old_burst_all_keyframe_bboxes_combined.csv"
DEFAULT_OUT_AUDIT = "/content/legacy_gt_support_audit.csv"
DEFAULT_OUT_MISSING = "/content/missing_old_burst_groups.csv"
DEFAULT_OUT_LINEAGE = "/content/legacy_source_trace_lineage.json"
...
def validate_output_paths(args, *, script_path: Path | None = None) -> dict[str, Path]:
    output_values = {
    ...
    project_root = (script_path or Path(__file__)).resolve().parent
    forbidden_roots = [
    ...
    for name, path in outputs.items():
        if any(_is_within(path, root) for root in search_roots):
            raise ValueError(
...
def validate_manifest_metadata(
    ...
    *,
    ...
    if "manifest__group_id" in combined.columns:
        present = combined["manifest__group_id"].map(_nonempty)
        mismatch = present & combined["manifest__group_id"].astype("string").ne(
            combined["group_id"].astype("string")
        )
        if mismatch.any():
            mismatch_keys = _sample_key_values(
                combined.loc[mismatch],
                ["img_name", "group_id", "manifest__group_id"],
            )
            raise ValueError(
                "manifest_group_id_mismatch="
        ...
        missing_files = sorted(
            {
                ...
                for value in combined.loc[resolved, "video_final"]
...
def main():
    ...
    output_paths = validate_output_paths(args)
