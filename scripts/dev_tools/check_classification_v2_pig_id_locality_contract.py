...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Check classification_v2 pig_id locality and leakage policy."
    )
    parser.add_argument(
        "--output-json",
        type=Path,
...
def check_pig_id_locality(paths: list[Path]) -> dict[str, Any]:
    ...
    return {
        ...
        "contracts_with_scope_hint": sum(
            1 for item in file_summaries if item.get("has_scope_hint")
        ),
        "forbidden_identity_allowance_count": sum(
            1 for item in file_summaries if item.get("has_forbidden_allowance")
        ),
