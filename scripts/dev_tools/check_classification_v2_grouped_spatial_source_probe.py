...
def main() -> None:
    ...
    for fold_number, fold_id in enumerate(sorted(roles["outer_fold_id"].astype(str).unique())):
        fold_roles = roles.loc[
            roles["outer_fold_id"].astype(str).eq(fold_id), ["temporal_unit_key", "role"]
        ]
...
def _validate_alignment(
    ...
    if not metadata["window_id"].astype(str).reset_index(drop=True).equals(
        events["window_id"].astype(str).reset_index(drop=True)
    ):
...
def _stratified_indices(
    frame: pd.DataFrame, mask: pd.Series, max_rows: int, seed: int
) -> np.ndarray:
...
def _flatten_mean_only(arrays: dict[str, np.ndarray], indices: np.ndarray) -> np.ndarray:
    ...
    return np.concatenate(
        [
            (arrays[name][indices] * observed[:, :, None]).sum(axis=1) / denominator
            for name in MODEL_GROUPS
        ],
        axis=1,
    )
