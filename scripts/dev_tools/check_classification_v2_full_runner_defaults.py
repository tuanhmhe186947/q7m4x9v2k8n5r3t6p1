...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Check classification_v2 full runner default config contract."
    )
    parser.add_argument(
        "--run-plan-json",
        type=Path,
...
def check_runner_defaults(run_plan_json: Path) -> dict[str, Any]:
    ...
    if config_hash != plan.get("config_sha256"):
        errors.append(
            "runner_default_fingerprint_mismatch="
            f"runner:{config_hash},plan:{plan.get('config_sha256')}"
        )
...
def _full_runner_default_config() -> FullMultimodalOofConfig:
    image_size = int(FULL_DEFAULTS["image_size"])
    return FullMultimodalOofConfig(
        output_dir=DEFAULT_FULL_OUTPUT_DIR,
        packed_image_cache_npy=_actor_packed_tensor(image_size),
        packed_image_cache_index_csv=DEFAULT_ACTOR_CACHE_ROOT
        / "packed_image_cache_index.csv",
        visual_context_cache_manifest_csv=DEFAULT_VISUAL_CONTEXT_MANIFEST,
        visual_context_packed_cache_npy=_visual_packed_tensor(image_size),
        visual_context_packed_cache_index_csv=DEFAULT_VISUAL_CACHE_ROOT
        / "packed_image_cache_index.csv",
