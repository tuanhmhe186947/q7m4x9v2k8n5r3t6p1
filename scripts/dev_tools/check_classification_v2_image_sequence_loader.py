...
def _read_cvat_crop(row: pd.Series, video_root: Path, video_index: dict[str, Path], cache: dict[str, Any], size: int) -> np.ndarray | None:
    if cv2 is None:
        return None
...
def _sample_windows(windows: pd.DataFrame, sample_per_source: int, *, include_invalid: bool) -> pd.DataFrame:
    ...
    samples = []
    for source_type, source in windows.groupby("source_type", sort=True):
        ...
        if len(source) <= sample_per_source:
            ...
            continue
        positions = sorted({round(i * (len(source) - 1) / max(1, sample_per_source - 1)) for i in range(sample_per_source)})
