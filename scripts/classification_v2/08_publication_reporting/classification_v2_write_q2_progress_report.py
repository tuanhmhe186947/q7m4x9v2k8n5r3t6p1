...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "outputs/classification_v2/model_smoke/training_reproducibility_cuda_post_s0a/reproducibility_audit.json"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--full-oof-authorization-file-json",
        type=Path,
        default=Path("outputs/classification_v2/model_design/full_oof_authorization_file_audit.json"),
    )
    parser.add_argument(
        "--full-oof-authorization-writer-json",
        type=Path,
        default=Path("outputs/classification_v2/model_design/full_oof_authorization_writer_audit.json"),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("outputs/classification_v2/model_design/full_oof_execution_gate_audit.json"),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("outputs/classification_v2/model_design/full_multimodal_oof_artifact_check.json"),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("outputs/classification_v2/model_design/image_cache_inventory_audit.json"),
    )
    parser.add_argument(
        "--image-cache-letterbox-policy-json",
        type=Path,
        default=Path("outputs/classification_v2/model_design/image_cache_letterbox_policy_audit.json"),
    )
    parser.add_argument(
        "--visual-interaction-cache-json",
        type=Path,
        default=Path("outputs/classification_v2/model_design/visual_interaction_cache_audit.json"),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("outputs/classification_v2/model_design/split_group_leakage_audit.json"),
    )
    parser.add_argument(
        "--interaction-context-index-json",
        type=Path,
        default=Path("outputs/classification_v2/model_design/interaction_context_index_audit.json"),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("outputs/classification_v2/train_ready_windows/spatial_sequence_audit.json"),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("outputs/classification_v2/train_ready_windows/image_sequence_loader_smoke_audit.json"),
    )
    parser.add_argument(
        "--output-json",
        type=Path,
    ...
    full_oof_authorization_policy = _load_optional_json(args.full_oof_authorization_policy_json)
    full_oof_authorization_template = _load_optional_json(args.full_oof_authorization_template_json)
    full_oof_authorization_file = _load_optional_json(args.full_oof_authorization_file_json)
    full_oof_authorization_writer = _load_optional_json(args.full_oof_authorization_writer_json)
    full_oof_preflight_freshness = _load_optional_json(args.full_oof_preflight_freshness_json)
    full_oof_run_plan = _load_optional_json(args.full_oof_run_plan_json)
    full_runner_default_config = _load_optional_json(args.full_runner_default_config_json)
    full_oof_execution_gate = _load_optional_json(args.full_oof_execution_gate_json)
    full_oof_launch_packet = _load_optional_json(args.full_oof_launch_packet_json)
    full_multimodal_oof_artifact_check = _load_optional_json(args.full_multimodal_oof_artifact_check_json)
    full_oof_completion_gate = _load_optional_json(args.full_oof_completion_gate_json)
    full_oof_postrun_registration_packet = _load_optional_json(args.full_oof_postrun_registration_packet_json)
    image_cache_inventory = _load_optional_json(args.image_cache_inventory_json)
    image_cache_letterbox_policy = _load_optional_json(args.image_cache_letterbox_policy_json)
    ...
    interaction_context_index = _load_optional_json(args.interaction_context_index_json)
    model_architecture_contract = _load_optional_json(args.model_architecture_contract_json)
    ablation_shortcut_contract = _load_optional_json(args.ablation_shortcut_contract_json)
    ablation_reporting = _load_optional_json(args.ablation_reporting_json)
    full_learned_oof_contract = _load_optional_json(args.full_learned_oof_contract_json)
    ...
    visual_context_ablation_smoke = _load_optional_json(args.visual_context_ablation_smoke_json)
    ...
    image_sequence_loader_audit = _load_optional_json(args.image_sequence_loader_audit_json)
    ...
    gates = [
        ...
        _gate(
            ...
            and pig_id_locality.get("contracts_with_pig_id") == pig_id_locality.get("contracts_with_scope_hint")
        ...
        _gate(
            ...
            and interaction_context_index.get("scene_partner_context_not_evaluated_count") == 0
            and interaction_context_index.get("window_uid_present") is False
            and interaction_context_index.get("forbidden_model_input_columns_present") == [],
        ...
        _gate(
            ...
            and ablation_shortcut_contract.get("planned_required_ablations_not_recorded") == [],
        ...
        _gate(
            ...
            and set(ablation_reporting.get("native_oof_comparable_ids", [])) >= {"B0", "B1", "B2"},
        ...
        _gate(
            ...
            and full_learned_oof_contract.get("external_generalization_claim") is False
            and full_learned_oof_contract.get("paper_ready") is False
            and (full_learned_oof_contract.get("required_record") or {}).get("exists") is False,
        ...
        _gate(
            ...
            and (data_module_audit.get("actor_image_load_audit") or {}).get("source_image_loads") == 0
            and (data_module_audit.get("visual_context_load_audit") or {}).get("individual_cache_loads") == 0,
        ...
        _gate(
            ...
            and q2_ablation_matrix.get("threshold_freeze_status") == "frozen_from_B4_inner_validation_seed_variance"
        ...
        _gate(
            ...
            and visual_context_ablation_smoke.get("full_visual_packed_hits", 0) > 0
            and visual_context_ablation_smoke.get("metric_interpretation")
        ...
        _gate(
            ...
            and loader_input_audit.get("source_domain_imbalanced_strata_after_count") == 0,
        ...
        _gate(
            ...
            and spatial_sequence_audit.get("observed_within_length_ratio", 0.0) > 0.99,
            spatial_sequence_audit.get("errors"),
        ),
        _gate(
        ...
        _gate(
            ...
            and full_oof_authorization_template.get("template_acknowledges_long_run_default") is False
            and full_oof_authorization_template.get("template_acknowledges_no_claim_default") is False,
            full_oof_authorization_template.get("errors"),
        ),
        _gate(
            "Full OOF authorization file fail-closed status",
            full_oof_authorization_file.get("valid") is True
            and full_oof_authorization_file.get("authorization_exists") is True
            and full_oof_authorization_file.get("binds_preflight_config_sha256") is True
            and full_oof_authorization_file.get("binds_git_commit") is True
            and full_oof_authorization_file.get("full_oof_execution_allowed") is False,
        ...
        _gate(
            ...
            and full_oof_authorization_writer.get("full_training_invoked") is False,
        ...
        _gate(
            ...
            and full_oof_preflight_freshness.get("full_oof_execution_allowed") is False
            and full_oof_preflight_freshness.get("preflight_authorization_ready") is True
            and full_oof_preflight_freshness.get("authorization_must_refresh_preflight") is False,
        ...
        _gate(
            ...
            and full_runner_default_config.get("runner_config_sha256") == full_oof_run_plan.get("config_sha256"),
        ...
        _gate(
            ...
            and full_oof_launch_packet.get("packet_status") == "READY_FOR_HUMAN_AUTHORIZATION"
            and full_oof_launch_packet.get("packet_matches_current_inputs") is True
            and full_oof_launch_packet.get("full_oof_execution_allowed") is False
            and full_oof_launch_packet.get("authorization_required") is True
            and full_oof_launch_packet.get("authorization_template_authorized") is False
            and _launch_packet_ready(full_oof_launch_packet)
            and full_oof_launch_packet.get("markdown_runbook_valid") is True
            and full_oof_launch_packet.get("markdown_overlong_command_line_count") == 0,
        ...
        _gate(
            ...
            and full_multimodal_oof_artifact_check.get("run_mode") in {"pilot", "full"}
            and full_multimodal_oof_artifact_check.get("prediction_schema_valid") is True
            and full_multimodal_oof_artifact_check.get("metrics_payload_valid") is True,
        ...
        _gate(
            ...
            and (full_oof_completion_gate.get("completion_ready") is full_oof_completion_gate.get("q2_claim_allowed")),
        ...
        _gate(
            ...
            and full_oof_postrun_registration_packet.get("packet_status") == "READY_FOR_POST_FULL_OOF_REGISTRATION"
            and full_oof_postrun_registration_packet.get("packet_matches_current_inputs") is True
            and full_oof_postrun_registration_packet.get("runs_training") is False
            and full_oof_postrun_registration_packet.get("runs_registration") is False
            and full_oof_postrun_registration_packet.get("q2_claim_allowed_by_packet") is False
            and _optional_true(
            ...
            and (full_oof_postrun_registration_packet.get("required_postrun_provenance_count") == 5)
            and _optional_true(
                full_oof_postrun_registration_packet,
            ...
            and (full_oof_postrun_registration_packet.get("parent_control_count") == 3)
            and (full_oof_postrun_registration_packet.get("parent_controls") is not None),
    ...
    remaining = [
        ...
        ("Full OOF runner requires authorization JSON bound to preflight config hash and Git commit."),
        ("Full OOF authorization template is intentionally non-authorized until a reviewer edits the approval fields."),
        (
            "Image cache inventory records non-canonical smoke/resume cache "
            "roots without deleting derived outputs automatically."
        ),
        ("Actor image cache policy is audited as letterbox aspect-preserving padding, not square stretch resizing."),
        (
    ...
    if not q2_feature_whitelist.get("valid"):
        remaining.append("Q2 feature whitelist or trainer input contract is not valid yet.")
    result = {
        ...
        "claim_boundary": (
            "Q2 internal recording-date/video-safe improvement only; no external farm/camera/cohort claim."
        ),
        "gates": gates,
        ...
        "evidence": {
            ...
            "image_cache_letterbox_policy": _evidence_image_cache_letterbox_policy(image_cache_letterbox_policy),
            "visual_interaction_cache": _evidence_visual_interaction_cache(visual_interaction_cache),
            "interaction_context_index": _evidence_interaction_context_index(interaction_context_index),
            "model_architecture_contract": _evidence_model_architecture_contract(model_architecture_contract),
            "ablation_shortcut_contract": _evidence_ablation_shortcut_contract(ablation_shortcut_contract),
            "ablation_reporting": _evidence_ablation_reporting(ablation_reporting),
            "full_learned_oof_contract": _evidence_full_learned_oof_contract(full_learned_oof_contract),
            "data_module": _evidence_data_module(data_module_audit),
            "training_config": _evidence_training_config(training_config_audit),
            "q2_ablation_matrix": _evidence_q2_ablation_matrix(q2_ablation_matrix),
            "visual_context_ablation_smoke": _evidence_visual_context_ablation_smoke(visual_context_ablation_smoke),
            "source_domain_control": _evidence_source_domain_control(source_domain_control),
            "loader_input_audit": _evidence_loader_input_audit(loader_input_audit),
            "spatial_sequence": _evidence_spatial_sequence(spatial_sequence_audit),
            "feature_semantics": _evidence_feature_semantics(feature_semantics_audit),
            "event_weight": _evidence_event_weight(event_weight_audit),
            "image_sequence_loader": _evidence_image_sequence_loader(image_sequence_loader_audit),
            "full_oof_preflight_policy": _evidence_full_oof_preflight_policy(full_oof_preflight_policy),
            "full_oof_authorization_policy": _evidence_full_oof_authorization_policy(full_oof_authorization_policy),
            "full_oof_authorization_template": _evidence_full_oof_authorization_template(
                full_oof_authorization_template
            ),
            "full_oof_authorization_file": _evidence_full_oof_authorization_file(full_oof_authorization_file),
            "full_oof_authorization_writer": (_evidence_full_oof_authorization_writer(full_oof_authorization_writer)),
            "full_oof_preflight_freshness": _evidence_full_oof_preflight_freshness(full_oof_preflight_freshness),
            "full_oof_run_plan": _evidence_full_oof_run_plan(full_oof_run_plan),
            "full_runner_default_config": _evidence_full_runner_default_config(full_runner_default_config),
            "full_oof_execution_gate": _evidence_full_oof_execution_gate(full_oof_execution_gate),
            "full_oof_launch_packet": _evidence_full_oof_launch_packet(full_oof_launch_packet),
            "full_multimodal_oof_artifact_check": (
                _evidence_full_multimodal_oof_artifact_check(full_multimodal_oof_artifact_check)
            ),
            "full_oof_completion_gate": _evidence_full_oof_completion_gate(full_oof_completion_gate),
            "full_oof_postrun_registration_packet": (
                _evidence_full_oof_postrun_registration_packet(full_oof_postrun_registration_packet)
            ),
...
def _has_case(audit: dict[str, Any], name: str) -> bool:
    ...
    if not isinstance(cases, list):
        return False
    return any(isinstance(case, dict) and case.get("name") == name for case in cases)
...
def _evidence_q2_active_review_contract(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        "valid": audit.get("valid"),
        "contract_version": audit.get("contract_version"),
        "active_review_can_apply_without_human_decision": audit.get("active_review_can_apply_without_human_decision"),
...
def _evidence_q2_feature_whitelist(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "implemented_spatial_model_groups": audit.get("implemented_spatial_model_groups"),
        "spatial_contract_model_groups": audit.get("spatial_contract_model_groups"),
        "spatial_model_group_contract_mismatch": audit.get("spatial_model_group_contract_mismatch"),
    }
...
def _evidence_pig_id_locality(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "forbidden_identity_allowance_count": audit.get("forbidden_identity_allowance_count"),
    }
...
def _evidence_split_group_leakage(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "split_group_key_uses_pig_id_only_count": audit.get("split_group_key_uses_pig_id_only_count"),
...
def _evidence_image_cache_inventory(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "ad_hoc_active_training_reference_count": audit.get("ad_hoc_active_training_reference_count"),
...
def _evidence_visual_interaction_cache(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "rows_dropped_for_missing_context": audit.get("rows_dropped_for_missing_context"),
    }
...
def _evidence_interaction_context_index(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "scene_partner_context_not_evaluated_count": audit.get("scene_partner_context_not_evaluated_count"),
        "model_input_feature_columns": audit.get("model_input_feature_columns"),
        "forbidden_model_input_columns_present": audit.get("forbidden_model_input_columns_present"),
    }
...
def _evidence_model_architecture_contract(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "missing_required_paper_branches": audit.get("missing_required_paper_branches"),
...
def _evidence_ablation_shortcut_contract(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "contract_version": audit.get("contract_version"),
        "paper_candidate_ready": audit.get("paper_candidate_ready"),
        "planned_required_ablations_not_recorded": audit.get("planned_required_ablations_not_recorded"),
        "paper_candidate_blockers": audit.get("paper_candidate_blockers"),
    }
...
def _evidence_ablation_reporting(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "external_generalization_claim": audit.get("external_generalization_claim"),
...
def _evidence_data_module(audit: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "duplicate_window_id": audit.get("duplicate_window_id"),
        "auxiliary_targets_not_model_inputs": audit.get("auxiliary_targets_not_model_inputs"),
        "actor_source_image_loads": actor_load.get("source_image_loads"),
        "actor_packed_cache_hits": actor_load.get("packed_image_cache_hits"),
        "visual_individual_cache_loads": visual_load.get("individual_cache_loads"),
...
def _evidence_q2_ablation_matrix(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "outer_test_execution_allowed": audit.get("outer_test_execution_allowed"),
        "threshold_freeze_status": audit.get("threshold_freeze_status"),
    }


def _evidence_visual_context_ablation_smoke(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        "valid": audit.get("valid"),
        "full_visual_packed_hits": audit.get("full_visual_packed_hits"),
        "full_trainable_parameter_count": audit.get("full_trainable_parameter_count"),
        "no_visual_trainable_parameter_count": audit.get("no_visual_trainable_parameter_count"),
...
def _evidence_source_domain_control(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "balanced_strata_after_count": audit.get("balanced_strata_after_count"),
        "imbalanced_strata_after_count": audit.get("imbalanced_strata_after_count"),
    }
...
def _evidence_loader_input_audit(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "tabular_feature_whitelist_count": audit.get("tabular_feature_whitelist_count"),
        "forbidden_x_columns": audit.get("forbidden_x_columns"),
        "tabular_x_columns_not_in_whitelist": audit.get("tabular_x_columns_not_in_whitelist"),
        "source_domain_kept_rows": audit.get("source_domain_kept_rows"),
        "source_domain_imbalanced_strata_after_count": audit.get("source_domain_imbalanced_strata_after_count"),
    }
...
def _evidence_feature_semantics(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "spatial_model_input_array_count": audit.get("spatial_model_input_array_count"),
        "spatial_model_input_role_errors": audit.get("spatial_model_input_role_errors"),
...
def _full_oof_run_plan_errors(plan: dict[str, Any]) -> list[str]:
    ...
    if int(plan.get("selected_fold_count") or 0) != int(plan.get("available_fold_count") or -1):
        errors.append(
            "full_oof_plan_selected_fold_count_mismatch="
            f"{plan.get('selected_fold_count')}!={plan.get('available_fold_count')}"
        )
    if int(plan.get("total_eval_rows") or 0) != int(load_audit.get("eligible_rows") or -1):
        errors.append(
            f"full_oof_plan_total_eval_rows_mismatch={plan.get('total_eval_rows')}!={load_audit.get('eligible_rows')}"
        )
...
def _evidence_full_oof_run_plan(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "visual_context_packed_cache_npy": config.get("visual_context_packed_cache_npy"),
        "visual_context_packed_cache_index_csv": config.get("visual_context_packed_cache_index_csv"),
...
def _evidence_full_runner_default_config(audit: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "require_packed_visual_context": runner_config.get("require_packed_visual_context"),
...
def _evidence_full_oof_execution_gate(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "case_names": [case.get("name") for case in audit.get("cases", []) or []],
        "errors": audit.get("errors"),
    }
...
def _launch_packet_ready(audit: dict[str, Any]) -> bool:
    ...
    if "launch_command_python_ready" in audit or "cmd_launch_command_ready" in audit:
        prefix_ready = (
            audit.get("cmd_launch_command_prefix_ready") if "cmd_launch_command_prefix_ready" in audit else True
        )
        bat_present = audit.get("cmd_launch_command_bat_present") if "cmd_launch_command_bat_present" in audit else True
        return bool(
...
def _evidence_full_oof_launch_packet(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "authorization_template_authorized": audit.get("authorization_template_authorized"),
        ...
        "cmd_launch_command_prefix_ready": audit.get("cmd_launch_command_prefix_ready"),
        "cmd_launch_command_bat_present": audit.get("cmd_launch_command_bat_present"),
        "authorization_command_ready": audit.get("authorization_command_ready"),
        "cmd_authorization_command_ready": audit.get("cmd_authorization_command_ready"),
        "cmd_authorization_command_wraps_base_command": audit.get("cmd_authorization_command_wraps_base_command"),
        "cmd_authorization_command_bat_present": audit.get("cmd_authorization_command_bat_present"),
        "estimated_training_seconds_excluding_eval": audit.get("estimated_training_seconds_excluding_eval"),
        "estimated_training_minutes_excluding_eval": audit.get("estimated_training_minutes_excluding_eval"),
        ...
        "markdown_wrapped_command_line_count": audit.get("markdown_wrapped_command_line_count"),
        "markdown_overlong_command_line_count": audit.get("markdown_overlong_command_line_count"),
    }


def _evidence_full_multimodal_oof_artifact_check(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        "valid": audit.get("valid"),
...
def _evidence_full_oof_postrun_registration_packet(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        "valid": audit.get("valid"),
        "packet_status": audit.get("packet_status"),
        "packet_matches_current_inputs": audit.get("packet_matches_current_inputs"),
        ...
        "python_executable": audit.get("python_executable"),
        "calibration_cmd_command_ready": audit.get("calibration_cmd_command_ready"),
        "calibration_command_bat_present": audit.get("calibration_command_bat_present"),
        "confusion_comparison_cmd_command_ready": audit.get("confusion_comparison_cmd_command_ready"),
        "confusion_comparison_command_bat_present": audit.get("confusion_comparison_command_bat_present"),
        "ablation_report_cmd_command_ready": audit.get("ablation_report_cmd_command_ready"),
        "ablation_report_command_bat_present": audit.get("ablation_report_command_bat_present"),
        ...
        "completion_gate_cmd_command_ready": audit.get("completion_gate_cmd_command_ready"),
        "completion_gate_command_bat_present": audit.get("completion_gate_command_bat_present"),
        "completion_gate_precheck_valid": audit.get("completion_gate_precheck_valid"),
        "completion_gate_precheck_fail_closed": audit.get("completion_gate_precheck_fail_closed"),
        "completion_gate_precheck_claim_allowed": audit.get("completion_gate_precheck_claim_allowed"),
        "required_artifact_count": audit.get("required_artifact_count"),
        "required_postrun_provenance_count": audit.get("required_postrun_provenance_count"),
        "register_postrun_provenance_paths_present": audit.get("register_postrun_provenance_paths_present"),
...
def _evidence_full_oof_authorization_policy(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "invalid_authorization_error_count": len(audit.get("invalid_authorization_errors", []) or []),
    }
...
def _evidence_full_oof_authorization_template(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "template_acknowledges_long_run_default": audit.get("template_acknowledges_long_run_default"),
        "template_acknowledges_no_claim_default": audit.get("template_acknowledges_no_claim_default"),
        "template_binds_preflight_config_sha256": audit.get("template_binds_preflight_config_sha256"),
        "template_binds_git_commit": audit.get("template_binds_git_commit"),
    }


def _evidence_full_oof_authorization_file(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        "valid": audit.get("valid"),
        "authorization_exists": audit.get("authorization_exists"),
        "authorized": audit.get("authorized"),
        "acknowledges_long_run": audit.get("acknowledges_long_run"),
        "acknowledges_no_q2_claim": audit.get("acknowledges_no_q2_claim"),
        "reviewer_present": audit.get("reviewer_present"),
        "binds_preflight_config_sha256": audit.get("binds_preflight_config_sha256"),
...
def _evidence_full_oof_preflight_freshness(audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "preflight_authorization_ready": audit.get("preflight_authorization_ready"),
        "full_oof_execution_allowed": audit.get("full_oof_execution_allowed"),
        "execution_blocked_reason": audit.get("execution_blocked_reason"),
        "authorization_must_refresh_preflight": audit.get("authorization_must_refresh_preflight"),
...
def _render_current_state_lines(evidence: dict[str, Any]) -> list[str]:
    ...
    return [
        ...
        f"- Launch runbook overlong command lines: `{launch.get('markdown_overlong_command_line_count')}`",
        f"- Postrun runbook Markdown valid: `{postrun.get('markdown_runbook_valid')}`",
        f"- Postrun runbook overlong command lines: `{postrun.get('markdown_overlong_command_line_count')}`",
...
def _render_next_action_lines(evidence: dict[str, Any]) -> list[str]:
    ...
    return [
        ...
        f"- No-Q2-claim acknowledged: `{authorization.get('acknowledges_no_q2_claim')}`",
        f"- Reviewer present: `{authorization.get('reviewer_present')}`",
        "- Review and run the authorization command template in:",
        "  `outputs/classification_v2/model_design/full_oof_launch_packet.md`",
        f"- Postrun packet ready: `{postrun.get('packet_status')}`",
    ]
