...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Check classification_v2 full OOF preflight freshness.")
    parser.add_argument(
        "--preflight-json",
        type=Path,
...
def check_preflight_freshness(
    *,
    ...
    git_matches = bool(git_state.get("commit") and git_state.get("commit") == preflight_git_commit)
    snapshot_matches = bool(current_snapshot_id and current_snapshot_id == preflight_snapshot_id)
    ...
    preflight_authorization_ready = bool(preflight_fresh and git_state.get("dirty") is False)
    if not preflight_valid:
        warnings.append(f"preflight_invalid={preflight.get('errors')}")
    if not git_matches:
        warnings.append(
            f"preflight_git_commit_stale=preflight:{preflight_git_commit},current:{git_state.get('commit')}"
        )
    if not snapshot_matches:
        warnings.append(f"preflight_snapshot_stale=preflight:{preflight_snapshot_id},current:{current_snapshot_id}")
    return {
