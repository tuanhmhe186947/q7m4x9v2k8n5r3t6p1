...
import write_classification_v2_full_oof_authorization_file as writer


def main() -> None:
    """Audit the full-OOF authorization writer without launching training."""

    parser = argparse.ArgumentParser(description="Check classification_v2 full OOF authorization writer.")
    parser.add_argument(
        "--preflight-json",
        type=Path,
...
def check_authorization_writer(preflight: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": ("classification_v2_full_oof_authorization_writer_audit_v1"),
...
def _draft_case(preflight: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "acknowledges_no_q2_claim": payload.get("acknowledges_no_q2_claim_until_verified"),
        "errors": errors,
    }
