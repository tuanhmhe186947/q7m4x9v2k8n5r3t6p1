...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Write classification_v2 full OOF post-run registration packet.")
...
def build_postrun_registration_packet(
    *,
    ...
    packet = {
        ...
        "required_postrun_provenance": {name: str(path) for name, path in postrun.items()},
        ...
        "calibration_command_bat": subprocess.list2cmdline(_cmd_command(calibration_command)),
        "confusion_comparison_command_bat": subprocess.list2cmdline(_cmd_command(confusion_command)),
        "ablation_report_command_bat": subprocess.list2cmdline(_cmd_command(ablation_command)),
        "register_command_bat": subprocess.list2cmdline(_cmd_command(register_command)),
        "completion_gate_command_bat": subprocess.list2cmdline(_cmd_command(completion_command)),
        "postrun_order": [
...
def render_postrun_packet_markdown(packet: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Calibration Command",
        "```bat",
        _wrap_bat_command_for_markdown(str(packet.get("calibration_command_bat") or "")),
        "```",
        "",
        "## Confusion Comparison Command",
        "```bat",
        _wrap_bat_command_for_markdown(str(packet.get("confusion_comparison_command_bat") or "")),
        "```",
        "",
        "## Ablation Report Command",
        "```bat",
        _wrap_bat_command_for_markdown(str(packet.get("ablation_report_command_bat") or "")),
        "```",
        "",
        "## Register Command",
        "```bat",
        _wrap_bat_command_for_markdown(str(packet.get("register_command_bat") or "")),
        "```",
        "",
        "## Completion Gate Command",
        "```bat",
        _wrap_bat_command_for_markdown(str(packet.get("completion_gate_command_bat") or "")),
        "```",
        "",
        "## Order",
    ]
...
def _register_command(
    *,
    ...
    command = [
        sys.executable,
        "scripts\\classification_v2\\08_publication_reporting\\classification_v2_register_experiment.py",
        "--name",
        "full_multimodal_oof",
        "--output-dir",
        str(registry_dir),
...
def _calibration_command(*, artifacts: dict[str, Path], output_dir: Path) -> list[str]:
    ...
    return [
        sys.executable,
        "scripts\\classification_v2\\07_postrun_evaluation\\classification_v2_cross_fit_calibration.py",
        "--input-csv",
...
def _ablation_report_command() -> list[str]:
    ...
    return [
        sys.executable,
        "scripts\\classification_v2\\07_postrun_evaluation\\check_classification_v2_ablation_reporting.py",
    ]
...
def _completion_gate_command(*, output_dir: Path, record_json: Path) -> list[str]:
    ...
    return [
        sys.executable,
        "scripts\\classification_v2\\09_final_release_audit\\check_classification_v2_full_oof_completion_gate.py",
        "--output-dir",
        str(output_dir),
