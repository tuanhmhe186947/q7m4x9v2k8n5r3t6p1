...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Check classification_v2 full OOF completion gate.")
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def build_completion_gate_audit(
    *,
    ...
    fail_closed = (completion_ready and q2_claim_allowed) or (not completion_ready and not q2_claim_allowed)
    if not fail_closed:
    ...
    return {
        ...
        "claim_boundary": (
            "Q2 internal recording-date/video-safe improvement only; external generalization claim remains false."
        ),
    }
...
def _validate_complete_artifacts(
    ...
    calibrated_schema_check = check_prediction_schema_csv(paths["calibrated_predictions"])
    ...
    if registry_contract.get("valid") is not True:
        blocking.append(f"registry_record_contract_invalid={registry_contract.get('errors')}")
    ...
    if schema_file_audit.get("valid") is not True:
        blocking.append(f"prediction_schema_file_invalid={schema_file_audit.get('errors')}")
    ...
    metrics_native_rows = (metrics.get("native_temporal_prediction_audit") or {}).get("native_temporal_unit_rows")
    if expected_native_rows != metrics_native_rows:
        blocking.append(f"native_temporal_row_count_mismatch={expected_native_rows}!={metrics_native_rows}")
    ...
    return {
        ...
        "confusion_paper_facing_inputs_verified": confusion.get("paper_facing_inputs_verified"),
        ...
        "calibrated_prediction_rows": calibrated_schema_check.get("prediction_rows"),
...
def _check_run_audit(
    ...
    if audit.get("git_commit") != preflight.get("git_commit"):
        blocking.append(f"run_preflight_git_commit_mismatch={audit.get('git_commit')}!={preflight.get('git_commit')}")
...
def _check_source_balanced_csvs(
    ...
    if "temporal_unit_key" in native.columns:
        ...
        if duplicate_native:
            blocking.append(f"source_balanced_native_duplicate_temporal_units={duplicate_native}")
    if "temporal_unit_key" in selection.columns:
        ...
        if duplicate_selection:
            blocking.append(f"source_balanced_selection_duplicate_temporal_units={duplicate_selection}")
    expected_rows = report.get("matched_native_unit_rows")
    if expected_rows is not None and int(expected_rows) != int(len(native)):
        blocking.append(f"source_balanced_native_row_count_mismatch={len(native)}!={expected_rows}")
    if "source_balance_keep" in selection.columns:
        ...
        if expected_rows is not None and keep_count != int(expected_rows):
            blocking.append(f"source_balanced_selection_keep_count_mismatch={keep_count}!={expected_rows}")
        if "temporal_unit_key" in native.columns:
            native_keys = set(native["temporal_unit_key"].fillna("").astype(str))
            selected_keys = set(selection.loc[keep_mask, "temporal_unit_key"].fillna("").astype(str))
...
def _check_registry_artifact_paths(
    ...
    artifact_paths = {
        _norm_path(artifact.get("path")) for artifact in record.get("artifacts", []) or [] if artifact.get("path")
    }
    required = (
        "run_audit",
        "predictions",
    ...
    missing = [name for name in required if _norm_path(paths[name]) not in artifact_paths]
    if missing:
...
def _check_calibrated_predictions_csv(
    ...
    if schema_check.get("valid") is not True:
        blocking.append(f"calibrated_prediction_schema_invalid={schema_check.get('errors')}")
    ...
    if expected_rows is not None and int(expected_rows) != int(len(frame)):
        blocking.append(f"calibrated_prediction_row_count_mismatch={len(frame)}!={expected_rows}")
...
def _check_hard_errors_csv(
    ...
    if expected_rows is not None and int(expected_rows) != int(len(frame)):
        blocking.append(f"hard_errors_row_count_mismatch={len(frame)}!={expected_rows}")
...
def _to_bool_series(series: pd.Series) -> pd.Series:
    ...
    return series.fillna("").astype(str).str.strip().str.lower().isin({"true", "1", "yes", "y", "t"})
...
def _expected_paths(output_dir: Path, registry_record: Path) -> dict[str, Path]:
    return {
        ...
        "prediction_schema_audit": (output_dir / "full_multimodal_oof_prediction_schema_audit.json"),
        "source_balanced_report": output_dir / "source_balanced_report.json",
        "source_balanced_native_units": (output_dir / "source_balanced_native_units.csv"),
        "source_balanced_selection": output_dir / "source_balanced_selection.csv",
        "calibrated_predictions": (output_dir / "calibration" / "cross_fitted_calibrated_native_predictions.csv"),
        "calibration_audit": (output_dir / "calibration" / "cross_fitted_calibration_audit.json"),
        "confusion_comparison": (output_dir / "confusion_focus" / "confusion_focus_comparison.json"),
        "high_confidence_hard_errors": (output_dir / "confusion_focus" / "high_confidence_hard_errors.csv"),
        "ablation_report": Path("outputs/classification_v2/model_design/ablation_reporting_audit.json"),
