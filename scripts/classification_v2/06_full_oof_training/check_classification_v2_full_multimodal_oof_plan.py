...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Planned model artifact directory. Defaults to model_full for full plans and model_smoke for pilot plans."
        ),
    )
    ...
    run_output_dir = args.run_output_dir or (DEFAULT_PILOT_OUTPUT_DIR if args.pilot else DEFAULT_FULL_OUTPUT_DIR)
    ...
    if not args.pilot:
        ...
        visual_manifest = visual_manifest or DEFAULT_VISUAL_CACHE_ROOT / ("visual_context_manifest.csv")
    ...
    config = FullMultimodalOofConfig(
        ...
        require_cached_images=(actor_manifest is not None or actor_tensor is not None),
