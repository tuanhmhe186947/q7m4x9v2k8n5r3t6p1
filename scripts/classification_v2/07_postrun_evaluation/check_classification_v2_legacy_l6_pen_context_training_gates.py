"""Write deterministic repeat or short-matrix pen-context gates."""
...
def main() -> int:
    ...
    config = load_pen_context_training_config(args.config)
    if args.command == "repeat":
        payload = write_pen_context_repeat_gate(
            config,
            mode=args.mode,
        ...
        payload = write_pen_context_short_matrix(
            config,
            repeat_gate_paths={
                mode: getattr(args, f"{mode}_gate") for mode in MODES
            },
            output_path=args.output,
        )
