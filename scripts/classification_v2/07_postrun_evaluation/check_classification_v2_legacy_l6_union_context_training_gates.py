"""Write repeat or complete short-matrix L6 union-context gates."""
...
from pig_behavior.classification_v2.training.legacy_development_l6_union_context_runtime import (
    write_union_context_repeat_gate,
    write_union_context_short_matrix,
)
...
def main() -> int:
    ...
    config = load_union_context_training_config(args.config)
    if args.command == "repeat":
        payload = write_union_context_repeat_gate(
            config,
            mode=args.mode,
        ...
        payload = write_union_context_short_matrix(
            config,
