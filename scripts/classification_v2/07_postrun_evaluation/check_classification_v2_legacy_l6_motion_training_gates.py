"""Write deterministic repeat or complete short-matrix L6 motion gates."""
...
from pig_behavior.classification_v2.training.legacy_development_l6_motion_runtime import (
    write_motion_repeat_gate,
    write_motion_short_matrix,
)
...
def main() -> int:
    ...
    config = load_motion_training_config(args.config)
    if args.command == "repeat":
        payload = write_motion_repeat_gate(
            config,
            mode=args.mode,
        ...
        payload = write_motion_short_matrix(
            config,
