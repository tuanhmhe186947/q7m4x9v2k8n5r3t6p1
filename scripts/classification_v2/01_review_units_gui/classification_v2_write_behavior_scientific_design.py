"""Write a pre-review behavior scientific design from audited artifacts."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--review-audit-json", type=Path, required=True)
    parser.add_argument(
        "--scientific-policy-json",
        type=Path,
        default=Path(
            "configs/classification_v2/"
            "behavior_review_scientific_policy_v1.json"
        ),
    )
...
def main() -> None:
    ...
    review_audit = json.loads(
        args.review_audit_json.read_text(encoding="utf-8")
    )
    if review_audit.get("errors"):
        raise ValueError(
            "Review-unit audit must pass before design declaration: "
            f"{review_audit['errors']}"
        )
    selection_contract = review_audit.get("behavior_selection", {})
    _, policy_payload, policy_sha256 = load_behavior_scientific_policy(
        args.scientific_policy_json
    )
    design = build_behavior_scientific_design(
        manifest,
    ...
    print(
        "[PASS] behavior scientific design "
