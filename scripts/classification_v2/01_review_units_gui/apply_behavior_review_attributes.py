...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="CSV from build_behavior_review_templates.py with auto review attributes.",
    )
    parser.add_argument(
        "--review-decisions-csv", type=Path, default=None, help="Filled review template or GUI decisions CSV."
    )
...
def apply_decisions(
    ...
    *,
    ...
    out.loc[has_manual_weight, "training_weight_final"] = out.loc[has_manual_weight, "manual_sample_weight_num"].clip(
        0, 1
    )
    ...
    if "roi_consistency_status_auto" in out.columns:
        ...
        if "use_for_roi_training" in out.columns:
            out["use_for_roi_training_final"] = (
                to_bool(out["use_for_roi_training"]) & ~unresolved & out["include_in_training_final"]
            )
        else:
