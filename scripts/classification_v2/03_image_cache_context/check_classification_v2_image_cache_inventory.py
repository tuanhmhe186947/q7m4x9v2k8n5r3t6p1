...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Inventory classification_v2 image cache roots.")
    parser.add_argument(
        "--root-dir",
        type=Path,
...
def check_image_cache_inventory(root_dir: Path) -> dict[str, Any]:
    ...
    if ad_hoc_dirs:
        warnings.append(f"non_canonical_image_cache_dirs_present={[path.as_posix() for path in ad_hoc_dirs]}")
    ...
    if active_reference_count:
        errors.append(f"non_canonical_cache_referenced_by_active_training_contracts={active_references}")
    ...
    return {
        ...
        "ad_hoc_cache_policy": (
            "residual_smoke_dirs_allowed_only_when_unreferenced_by_active_training_configs_or_snapshots"
        ),
        "warnings": warnings,
        "errors": errors,
        "valid": not errors,
