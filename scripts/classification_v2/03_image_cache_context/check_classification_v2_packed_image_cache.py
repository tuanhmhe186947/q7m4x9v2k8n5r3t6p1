...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Check packed image-cache lineage and pixel equivalence."
    )
    parser.add_argument(
        "--root",
        type=Path,
...
def check_packed_cache(
    ...
    if len(manifest) != len(index) or len(index) != tensor.shape[0]:
        errors.append(
            f"row_count_mismatch=manifest:{len(manifest)} "
            f"index:{len(index)} tensor:{tensor.shape[0]}"
        )
    if (
        manifest["image_context_id"].astype(str).tolist()
        != index["image_context_id"].astype(str).tolist()
    ):
        errors.append("ordered_image_context_id_mismatch")
    expected_rows = np.arange(len(index), dtype=np.int64)
    packed_rows = pd.to_numeric(index["packed_row"]).to_numpy(dtype=np.int64)
    if not np.array_equal(packed_rows, expected_rows):
