...
from pig_behavior.classification_v2.training.legacy_development_l6_roi_relation_cache import (
    ...
    build_roi_relation_cache,
    load_roi_relation_cache_config,
    preflight_roi_relation_cache,
)
...
def main() -> None:
    ...
    config = load_roi_relation_cache_config(args.config_json)
    if args.action == "preflight":
        result = preflight_roi_relation_cache(config)
    elif args.action == "build":
        path, result = build_roi_relation_cache(config)
        result = {**result, "manifest_path": str(path)}
    else:
        result = audit_roi_relation_cache(config)
