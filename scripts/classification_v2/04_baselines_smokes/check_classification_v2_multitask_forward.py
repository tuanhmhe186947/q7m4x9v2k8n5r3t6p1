...
def main() -> None:
    ...
    head_gradients = {name: _gradient_sum(head) for name, head in model.auxiliary_heads.heads.items()}
    errors: list[str] = []
    ...
    behavior_only_auxiliary_shapes = {
        name: list(logits.shape) for name, logits in behavior_only_output.auxiliary_logits().items()
    }
