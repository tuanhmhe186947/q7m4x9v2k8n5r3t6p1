"""Preflight, run, or audit one legacy L6 geometry control."""
...
def main() -> int:
    ...
    if args.command == "preflight":
        payload = preflight_geometry_mode(config, args.mode)
    elif args.command == "run":
        payload = run_geometry_mode(
            config,
            mode=args.mode,
            run_id=args.run_id,
        )
    else:
        payload = audit_geometry_run(config, result_path=args.result)
