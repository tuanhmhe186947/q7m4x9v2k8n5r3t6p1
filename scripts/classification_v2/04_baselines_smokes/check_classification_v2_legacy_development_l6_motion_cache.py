...
def main() -> None:
    ...
    config = load_motion_cache_config(args.config_json)
    if args.action == "preflight":
        result = preflight_motion_cache(config)
    elif args.action == "build":
        path, result = build_motion_cache(config)
        result = {**result, "manifest_path": str(path)}
    else:
        result = audit_motion_cache(config)
