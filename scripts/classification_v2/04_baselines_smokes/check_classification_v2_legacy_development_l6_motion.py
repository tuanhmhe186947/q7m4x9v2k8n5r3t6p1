"""Preflight, run, or audit one legacy L6 motion control."""
...
def main() -> int:
    ...
    if args.command == "preflight":
        payload = preflight_motion_mode(config, args.mode)
    elif args.command == "run":
        payload = run_motion_mode(
            config,
            mode=args.mode,
            run_id=args.run_id,
        )
    else:
        payload = audit_motion_run(config, result_path=args.result)
