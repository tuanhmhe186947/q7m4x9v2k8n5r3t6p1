...
def main() -> None:
    """Validate B1 artifacts before they are accepted as paper-facing evidence."""

    parser = argparse.ArgumentParser(description="Check classification_v2 B1 linear tabular baseline artifacts.")
    parser.add_argument(
        "--audit-json",
        type=Path,
        default=Path("outputs/classification_v2/model_smoke/tabular_linear_baseline/tabular_linear_audit.json"),
    )
    parser.add_argument(
        "--metrics-json",
        type=Path,
        default=Path("outputs/classification_v2/model_smoke/tabular_linear_baseline/tabular_linear_metrics.json"),
    )
    parser.add_argument(
        "--predictions-csv",
        type=Path,
        default=Path("outputs/classification_v2/model_smoke/tabular_linear_baseline/tabular_linear_predictions.csv"),
    )
...
def _read_json(path: Path, errors: list[str], name: str) -> dict:
    """Read a JSON evidence file while preserving missing-file errors for audit output."""
