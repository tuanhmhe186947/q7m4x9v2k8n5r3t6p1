"""Preflight, run, or audit one legacy L6 union-context control."""
...
def main() -> int:
    ...
    if args.command == "preflight":
        payload = preflight_union_context_mode(config, args.mode)
    elif args.command == "run":
        payload = run_union_context_mode(
            config,
            mode=args.mode,
            run_id=args.run_id,
        )
    else:
        payload = audit_union_context_run(
            config,
