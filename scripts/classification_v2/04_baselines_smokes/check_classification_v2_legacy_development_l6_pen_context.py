"""Preflight, run, or audit one legacy pen-context control."""
...
def main() -> int:
    ...
    if args.command == "preflight":
        payload = preflight_pen_context_mode(config, args.mode)
    elif args.command == "run":
        payload = run_pen_context_mode(
            config,
            mode=args.mode,
            run_id=args.run_id,
        )
    else:
        payload = audit_pen_context_run(config, result_path=args.result)
