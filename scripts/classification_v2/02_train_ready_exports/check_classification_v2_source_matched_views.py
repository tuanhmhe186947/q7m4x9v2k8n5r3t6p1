...
def main() -> None:
    ...
    changed = joined["behavior_window_label_before"].astype(str).ne(joined["behavior_window_label_after"].astype(str))
    if changed.any():
    ...
    unequal = {str(label): counts.to_dict() for label, counts in contingency.groupby(level=0) if counts.nunique() != 1}
    if unequal:
