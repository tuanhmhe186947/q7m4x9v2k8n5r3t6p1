...
def _parse_ratios(text: str) -> dict[str, float]:
    ...
    return {name: value / total for name, value in zip(SPLITS, parts, strict=False)}
    ...
    for group in ordered_groups:
        best_split = min(
            ...
            key=lambda split: (
                ...
                max(
                    0.0,
                    (assigned_rows[split] + group["balance_rows"] - target_rows[split]) / max(1.0, target_rows[split]),
                ),
