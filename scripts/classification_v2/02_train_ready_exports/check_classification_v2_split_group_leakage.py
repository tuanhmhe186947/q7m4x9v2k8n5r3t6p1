...
DEFAULT_SPLIT_MANIFEST = Path("outputs/classification_v2/train_ready_windows/split_manifest.csv")
DEFAULT_SPLIT_AUDIT = Path("outputs/classification_v2/train_ready_windows/split_audit.json")
DEFAULT_OUTPUT_JSON = Path("outputs/classification_v2/model_design/split_group_leakage_audit.json")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Audit classification_v2 split artifacts for group leakage.")
...
def check_split_group_leakage(manifest_path: Path, split_audit_path: Path) -> dict[str, Any]:
    ...
    if split_audit.get("leakage_group_count", 0) not in (0, None):
        errors.append(f"builder_split_audit_leakage_group_count={split_audit.get('leakage_group_count')}")
    ...
    return {
        ...
        "group_split_leakage_count": _group_leakage_count(manifest, "split_group_key", "split"),
        "video_split_leakage_count": _group_leakage_count(manifest, "video_key", "split"),
        "split_group_key_uses_pig_id_only_count": _pig_id_only_group_key_count(manifest),
