...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export leakage-safe X/y/mask/sample_weight tables from reviewed sequence windows."
    )
    parser.add_argument(
        "--input-csv",
        type=Path,
        default=Path("outputs/classification_v2/sequence_features_reviewed/sequence_window_features.csv"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def main() -> None:
    ...
    print(f"[OK] wrote {x_path} rows={len(tables.x)} cols={len(tables.x.columns)}")
    print(f"[OK] wrote {y_path} rows={len(tables.y)}")
    print(f"[OK] wrote {mask_path} true={int(tables.mask.sum())} false={int((~tables.mask).sum())}")
