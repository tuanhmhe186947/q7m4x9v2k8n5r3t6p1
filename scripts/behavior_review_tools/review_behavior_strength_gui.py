...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "CSV storing/reusing manual decisions. Default: "
            "<output-dir>/behavior_strength_review_decisions.csv"
        ),
    )
    parser.add_argument(
        "--merged-review-csv",
...
class BehaviorStrengthReviewApp:
    ...
    def _build_ui(self) -> None:
        ...
        tk.Button(controls, text="Save decision [Enter]", command=self.save_current, bg="#d7ffd7", height=2).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)
        tk.Button(controls, text="Skip [S/Right]", command=self.skip, bg="#eeeeee", height=2).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)
        tk.Button(controls, text="Undo current session [U]", command=self.undo, bg="#eeeeff", height=2).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)
        tk.Button(controls, text="Save progress [Ctrl+S]", command=self.save_progress, bg="#d7e8ff", height=2).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)
        tk.Button(controls, text="Save & Exit [Esc]", command=self.confirm_exit, bg="#ffeecf", height=2).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)

        tk.Label(self.root, textvariable=self.status_var, justify=tk.LEFT, anchor="w", font=("Consolas", 10)).pack(side=tk.BOTTOM, fill=tk.X, padx=10, pady=4)
    ...
    def _combo(self, parent: tk.Frame, label: str, var: tk.StringVar, values: list[str], col: int) -> None:
        ...
        ttk.Combobox(parent, textvariable=var, values=values, width=20, state="readonly").grid(row=1, column=col * 2, padx=4, sticky="ew")
    ...
    def _show_current(self) -> None:
        ...
        self.info_var.set("\n".join([
            f"Item {self.index + 1}/{len(self.items)} | already saved decisions={len(self.decisions_by_key)} | source={item.image_source}",
            ...
            f"video={item.video_key} | frame={item.frame_index} | rel={item.relative_frame_index} | frame_uid={item.frame_uid}",
            f"track={item.track_id} | pig={item.pig_id} | review_key={item.review_key} | review_row_index={item.review_row_index}",
            f"bbox=({item.x1:.1f}, {item.y1:.1f}, {item.x2:.1f}, {item.y2:.1f})",
            "Keys: 1 strong, 2 medium, 3 weak, 4 boundary, Enter save, S skip, Ctrl+S save progress, Esc save exit",
        ]))

        self._show_image(item)
        self.status_var.set(
            f"Session decisions: {len(self.session_order)} | Total saved-map decisions: {len(self.decisions_by_key)} | Remaining this run: {len(self.items) - self.index}"
        )
    ...
    def _copy_image(self, decision: ReviewDecision) -> None:
        dst_dir = self.output_dir / "reviewed_images" / sanitize(decision.manual_label_strength) / sanitize(decision.behavior)
    ...
    def _save_decisions(self, *, show_message: bool) -> None:
        ...
        rows.sort(key=lambda r: (str(r.get("video_key", "")), safe_int(r.get("frame_index", ""), 0), str(r.get("pig_id", "")), str(r.get("review_key", ""))))
...
def default_action(strength: str) -> str:
    return {"strong": "main_train", "medium": "main_train", "weak": "low_weight_train", "boundary": "exclude"}.get(strength, "")
