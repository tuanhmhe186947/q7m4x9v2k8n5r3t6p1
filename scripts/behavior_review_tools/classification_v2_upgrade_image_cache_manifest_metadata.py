...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Upgrade classification_v2 image cache manifest metadata."
    )
    parser.add_argument(
        "--manifest-csv",
        type=Path,
    ...
    parser.add_argument(
        ...
        default=Path(
            ...
            "manifest_metadata_upgrade_audit.json"
        ),
    )
...
def _compute_letterbox_metadata(manifest: pd.DataFrame) -> pd.DataFrame:
    ...
    out.loc[valid, "source_crop_aspect_ratio"] = (
        crop_width[valid] / crop_height[valid]
    ).astype(float)
    ...
    out.loc[valid, "_padded_canvas"] = (
        pad_left.gt(0) | pad_top.gt(0) | pad_right.gt(0) | pad_bottom.gt(0)
    )
    return out
