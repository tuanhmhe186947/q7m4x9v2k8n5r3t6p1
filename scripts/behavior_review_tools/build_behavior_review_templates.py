...
def add_motion_and_boundary_features(
    ...
    *,
    ...
    prev_changed = df["prev_behavior_auto"].notna() & (df["prev_behavior_auto"].astype(str) != df["behavior"].astype(str))
    next_changed = df["next_behavior_auto"].notna() & (df["next_behavior_auto"].astype(str) != df["behavior"].astype(str))
