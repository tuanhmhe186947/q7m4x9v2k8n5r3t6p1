...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--csv", type=Path, action="append", default=[], help="Crop/feature CSV to review. Can be repeated.")
    parser.add_argument("--raw-root", type=Path, action="append", default=[], help="Root containing crop images. Can be repeated.")

    parser.add_argument("--xml", type=Path, action="append", default=[], help="Single CVAT XML file. Can be repeated.")
    parser.add_argument("--xml-dir", type=Path, action="append", default=[], help="Folder containing CVAT XML files. Can be repeated.")
    parser.add_argument("--video-root", type=Path, action="append", default=[], help="Root containing videos. Can be repeated.")

    parser.add_argument("--output-dir", type=Path, required=True, help="Output folder for decisions/progress/corrected copies.")
    parser.add_argument("--padding", type=float, default=0.12, help="Padding ratio around bbox for video crops. Default: 0.12.")
    parser.add_argument("--include-hidden-no", action="store_true", help="Also review rows already marked Hidden=No. Default reviews Hidden=Yes only.")
    parser.add_argument("--max-items", type=int, default=None, help="Limit remaining review items.")
    parser.add_argument("--copy-reviewed-crops", action="store_true", help="Save/copy reviewed crop images to output-dir/reviewed_crops.")
    parser.add_argument("--overwrite", action="store_true", help="Overwrite XML files. Default saves corrected XML copies only.")
    ...
    parser.add_argument("--include-reviewed", action="store_true", help="Show rows already present in hidden_review_decisions.csv.")
    parser.add_argument("--no-resume", action="store_true", help="Ignore existing hidden_review_decisions.csv and start a new decision map.")
    parser.add_argument("--autosave-corrected", action="store_true", help="Also write corrected CSV/XML copies after every decision. Slower but safest.")
...
class UnifiedHiddenReviewApp:
    ...
    def _build_ui(self) -> None:
        ...
        tk.Label(top, textvariable=self.info_var, justify=tk.LEFT, anchor="w", font=("Consolas", 10)).pack(side=tk.LEFT, fill=tk.X, expand=True)
        ...
        tk.Button(controls, text="✓ Hidden đúng - giữ Yes [V]", command=self.keep_hidden, bg="#d7ffd7", font=("Arial", 12, "bold"), height=2).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)
        tk.Button(controls, text="✗ Không hidden - đổi No [X]", command=self.mark_not_hidden, bg="#ffd7d7", font=("Arial", 12, "bold"), height=2).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)
        tk.Button(controls, text="Skip [S/Right]", command=self.skip, bg="#eeeeee", font=("Arial", 12), height=2).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)
        tk.Button(controls, text="Undo [U]", command=self.undo, bg="#eeeeff", font=("Arial", 12), height=2).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)
        tk.Button(controls, text="Save & Exit [Ctrl+S]", command=self.save_and_exit, bg="#d7e8ff", font=("Arial", 12, "bold"), height=2).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)

        tk.Label(self.root, textvariable=self.status_var, justify=tk.LEFT, anchor="w", font=("Consolas", 10)).pack(side=tk.BOTTOM, fill=tk.X, padx=10, pady=6)
    ...
    def _show_current(self) -> None:
        ...
        previous_text = ""
        if previous:
            previous_text = f"previous_decision={previous.get('decision', '')} -> new_hidden={previous.get('new_hidden', '')}"

        self.info_var.set(
            "\n".join(
                [
                    f"Item: {self.index + 1}/{len(self.items)} | reviewed_map={len(self.decisions_by_key)}/{len(self.all_items)}",
                    ...
                    f"video_key={item.video_key} | frame={item.frame_index} | rel={item.relative_frame_index} | frame_uid={item.frame_uid}",
        ...
        self.status_var.set(
            f"Session decisions: {len(self.session_stack)} | Remaining this run: {len(self.items) - self.index} | Decision file: {self.output_dir / DECISIONS_CSV_NAME}"
        )
    ...
    def _save_reviewed_crop(self, item: ReviewItem, record: ReviewDecision) -> None:
        if self.current_image_pil is None:
            return
        out_dir = self.output_dir / "reviewed_crops" / sanitize(record.decision) / sanitize(record.video_key or "no_video_key")
...
def apply_hidden_to_source(item: ReviewItem, new_hidden: str, dataframes: dict[Path, pd.DataFrame]) -> None:
    if item.source_kind == "csv_crop":
        ...
        if new_hidden == "No":
            ...
            if "training_tier" in df.columns and str(df.loc[item.row_index, "training_tier"]).lower() in {"review", "warning"}:
...
def write_decisions_csv(path: Path, decisions_by_key: dict[str, dict[str, str]]) -> None:
    ...
    rows.sort(key=lambda r: (r.get("source_kind", ""), r.get("source_path", ""), r.get("frame_index", ""), r.get("item_key", "")))
    ...
    for csv_path in csv_paths:
        ...
        for row_index, row in df[mask].iterrows():
            ...
            if image_path is None:
                missing.append(row_to_missing_record("csv_crop", csv_path, int(row_index), row, reason="missing_crop_image"))
                continue
...
def collect_xml_items(
    ...
    for xml_path in xml_files:
        ...
        if video_path is None:
            missing.append({"source_kind": "cvat_xml_video", "source_path": str(xml_path), "reason": "missing_video", "task_name": task_name, "video_key": video_key})
            continue
...
def resolve_crop_image(row: pd.Series, image_index: dict[str, list[Path]]) -> Path | None:
    candidates: list[str] = []
    for col in ["crop_path", "image_path", "crop_image_path", "review_image_path", "image_name", "image_key", "frame_uid"]:
...
def make_xml_item_key(xml_path: Path, track_id: str, frame_index: int, pig_id: str, behavior: str, x1: float, y1: float, x2: float, y2: float) -> str:
    parts = ["cvat_xml_video", str(xml_path.resolve()), str(track_id), str(frame_index), str(pig_id), str(behavior), f"{x1:.2f}", f"{y1:.2f}", f"{x2:.2f}", f"{y2:.2f}"]
    return normalize_key("|".join(parts))


def keys_for_path(path: Path, root: Path) -> set[str]:
    keys = {str(path), path.name, path.stem, normalize_key(str(path)), normalize_key(path.name), normalize_key(path.stem)}
    try:
...
def row_to_missing_record(source_kind: str, source_path: Path, row_index: int, row: pd.Series, *, reason: str) -> dict[str, Any]:
    return {
