...
Verified by a **real bounded production export** at the current Git SHA (real producer `build_enhanced_spatiotemporal_features` + real exporter `export_spatial_sequences` on a real 2-unit harmonized-frame slice; see `motion_dimension_contract.json`). These are the model-input group widths `D` (T padded to 16; primary view T6); re-assert on the reviewed export via the loader-contract audit (§Phase 1):
...
| `motion_delta` | **12** | per-second motion `motion_tensor.v2` (`vx,vy,bw_rate,bh_rate,area_rate,aspect_rate,speed,dir_change,tang_acc,ax,ay,acc_mag`); **12 confirmed end-to-end by real bounded export** (NPZ `[1,6,12]`, preflight `[]`) |
...
| **spatial predictive total** | **46** | `4+2+12+18+10` (excludes quality_mask); corrected from 44 (legacy 10-D motion) |
...
- **Severe date skew:** 3 dates hold **89%** of units (291119 44%, 301119 29%, 281119 16%); 11 legacy tokens hold 11%. Naive **full** leave-one-date-out over all 13 groups is unbalanced and low-power. **Outer protocol = `FOUR_FOLD_DATE_GROUPED_OUTER_CV`** (this is **NOT** full leave-one-date-out): F1 holds out `291119`, F2 holds out `301119`, F3 holds out `281119`, and **F4 pools all 11 small legacy tokens**; every fold is date-grouped so no calendar date spans train/test. **Descriptive per-date metrics are still reported for all 13 groups** (support + CI), but only these 4 folds carry the generalization estimate. **Effective outer generalization units ≈ 4 → low statistical power (stated as a limitation).**
...
- **`HISTORICAL_C6_SCREEN`** — historical legacy-only development evidence built from **legacy 16-frame offsets `[5,6,7,8,9,10]`** (the middle-6 screen), distinct from the `[0,3,6,9,12,15]` offsets of `S6_AT_16_SPARSE` and from any contiguous view; **not** equivalent to `T6_CONTIGUOUS` or `S6_AT_16_SPARSE`; neither its metrics nor its name transfer to the mixed reviewed lineage (`HISTORICAL_C6_METRICS_TRANSFERRED = NO`, `HISTORICAL_C6_NAME_TRANSFERRED = NO`).
...
**Cross-length study (Phase 7b) — two orthogonal families, never conflated.** Run **only after** the provisional winning loss is selected on a fixed `T6_CONTIGUOUS` baseline. Hold constant the snapshot, folds, backbone, modality set, fusion, loss, optimizer, schedule, resolution, seeds, metric, and collapse rule.

- **`TARGET_VIEW_ABLATION`** — vary the *labelled prediction window* length: `T6 / T8 / T12 / T16_TARGET_CONTIGUOUS`. Exactly **one** label per window (the target native unit's final behavior). This asks *how much of the current event to observe*.
- **`CAUSAL_HISTORY_ABLATION`** — fix the target at `T6` and prepend **causal history preceding the target**: `T6_TARGET_PLUS_H6 / _PLUS_H12 / _PLUS_H24`. The label comes from the **T6 target only**; history-frame labels are **never** placed in X, and no history frame is later than the target endpoint (`FUTURE_FRAME_DEPENDENCE = 0`). This asks *how much preceding context helps*.

These two families are reported and adjudicated **separately** — a target-window gain and a causal-history gain are different scientific claims and must not be pooled.

**Two evaluation populations for every length comparison (target-view or history).** Report both `ALL_ELIGIBLE` (all native units/windows eligible for that specific length) and `COMMON_MATCHED_COHORT` (the native units eligible across **all** compared lengths). **Do not claim any length gain unless it survives the `COMMON_MATCHED_COHORT`** — an `ALL_ELIGIBLE`-only difference may be a support/population artifact of longer views having fewer eligible windows. Apply the overlapping-window **training-mass correction** (fixed windows per native-unit, or `sample_weight = native_unit_class_weight / windows_from_same_native_unit`; native-unit mass audited before/after) before fitting, so a longer view is not credited merely for yielding more windows per native unit.

For each view report: eligible window count, native-unit coverage, class distribution, source distribution, physical-span distribution (timestamps present: `timestamp_start_sec`/`timestamp_end_sec`), effective observation rate, adjacent-pair coverage, missingness, macro-F1-supported (**both** populations), balanced accuracy, per-class F1/recall, calibration, confusion-pair metrics, latency, peak VRAM, parameter count, and **source-decode accuracy from view metadata**. **Do not** conclude a longer view is better from window-level accuracy — longer views may reduce eligible support, cross more label boundaries, and increase imbalance/latency/memory.
...
MOTION_DIMENSION_CONTRACT=PASS (real bounded production export; motion_dimension_contract.json)
CURRENT_CANONICAL_MOTION_DIMENSION=12
MOTION_DELTA_DIM=12
SPATIAL_PREDICTIVE_TOTAL=46
END_TO_END_12D=YES (declared==producer==exporter==loader==real bounded NPZ==12; preflight [])
HISTORICAL_MOTION_ARTIFACTS_REUSABLE=NO (9D/10D are STALE_PRE_MOTION_V2; rebuild, never pad)
MODEL_IMPLEMENTATION_ALLOWED=YES
...
OUTER_PROTOCOL_NAME=FOUR_FOLD_DATE_GROUPED_OUTER_CV (NOT full leave-one-date-out; F1=291119, F2=301119, F3=281119, F4=pooled 11 small legacy tokens; per-date descriptive for all 13)
...
TARGET_VIEW_ABLATION=T6|T8|T12|T16_TARGET_CONTIGUOUS (single label per window)
CAUSAL_HISTORY_ABLATION=T6_TARGET_PLUS_H6|H12|H24 (label from T6 target only; history labels never in X)
CROSS_LENGTH_FAMILIES_SEPARATED=YES (target-view length vs causal-history length are distinct claims, never pooled)
EVAL_POPULATIONS=ALL_ELIGIBLE|COMMON_MATCHED_COHORT (no length gain credited unless it survives COMMON_MATCHED_COHORT)
TRAINING_MASS_CORRECTION=REQUIRED (fixed windows/native-unit or sample_weight=native_unit_class_weight/windows_from_same_native_unit; native-unit mass audited before/after)
...
LEGACY_ONLY_SPARSE_ABLATION=S6_AT_16_SPARSE (offsets [0,3,6,9,12,15] in one legacy 16-frame burst)
HISTORICAL_C6_SCREEN_OFFSETS=[5,6,7,8,9,10] (legacy middle-6; historical only)
S6_AT_16_SPARSE_OFFSETS=[0,3,6,9,12,15]
HISTORICAL_C6_METRICS_TRANSFERRED=NO
HISTORICAL_C6_NAME_TRANSFERRED=NO
